...
\begin{exerc}
  We have observed that unlike the nowhere dense property, the meager property is not necessarily preserved to the closure. Prove that if $\bar{A}$ is meager, then $A$ was nowhere dense to begin with.
\end{exerc}
...
\begin{proof}[Proof of conditions (a) and (b) only]
  For condition (a), let $I$ be an interval. Since $I$ is an interval we clearly have $m(I)\leq\ell(I)$. For the other direction, we will show that $I\subset\bigcup I_n$ implies $\sum\ell(I_n)\geq\ell(I)$. Then taking the infemum over all such coverings this allows us to conclude that $m(I)\geq\ell(I)$.
...
The notion of null set bears many similarities with the notion of meager set. As was the case with meager sets, the notion of a null set allows us to assign to any given set $A$ one of three simple ``sizes'': null, conull, or non-null and non-conull. Moreover, the analog of the Baire category theorem holds for null sets as well.
...
Of course this follows from Theorem~\ref{thm:caratheodory} above, since in particular the measure function satisfies condition (a).

\subsection*{Exercises}

\begin{exerc}
  Give a proof that the Cantor set is null using Definition~\ref{defn:null} directly.
\end{exerc}
