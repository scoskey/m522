...
\begin{proof}[Proof of Lemma~\ref{lem:null-slalom}]
  ...
  To define $\phi$, we fix a sequence of subsets $E_{n,i}\subset2^\omega$ such that $m(E_{n,i})=2^{-n}$. Given $f\in\omega^\omega$ we define
  \[\phi(f)=\bigcap_N\bigcup_{n\geq N} E_{n,f(n)}\;.
  \]
  Then it is clear that $\phi(f)$ is a null set. For technical reasons later on, we will need to assume that the sets $E_{n,i}$ are mutually independent events (the measure of the intersection is the product of the measures).

  Before defining $\psi$, given a null set $A\subset2^\omega$ let us preview what properties we will need of the slalom $\psi(A)$. So suppose that $f$ is given and $\phi(f)\subset A$. Let $K$ be chosen as in Proposition~\ref{prop:self-supporting}, so we have $K\cap\bigcap_N\bigcup_{n\geq N}E_{n,f(n)}=\emptyset$. Since $K$ is complete and the sets $\bigcup_{n\geq N}E_{n,f(n)}$ are open, the Baire category theorem implies that at least one of them is not dense. Thus there is a basic open set $V_s$ such that $V_s\cap K\neq\emptyset$ and eventually we have $V_s\cap K\cap E_{n,f(n)}=\emptyset$. We will define the slalom $\psi(A)=(I_n)$ so that for $n$ large enough, $I_n$ it includes all such $f(n)$. This will guarantee that $f\in^*\psi(A)$ as required.
  ...
  \begin{itemize}
  \item We need a slalom, that is, we need to ensure $|(I_n(s))|\leq2^n$
  \item We need a single slalom $(I_n)$ that handles all $s$ at once.
  \end{itemize}

  For the first issue, we claim that the sum $\sum2^{-n}|I_n(s)|$ converges. Indeed, by the independence of the sets $E_{n,i}$ we have
  ...
  To quickly recapitulate why this works, again suppose that $\phi(f)\subset A$. By the above reasoning we can find $V_s$ such that $V_s\cap K\neq\emptyset$ and eventually we have $V_s\cap K\cap E_{n,f(n)}=\emptyset$. Thus for $n$ large enough we have $f(n)\in I_n(s)$ and $I_n(s)\subset I_n$. It follows that $f\in^*\psi(A)$.
\end{proof}
...
This time we will need a topological fact before the proof.

\begin{prop}
  \label{prop:pi-base-meager}
  Given any $n\in\NN$, there is a family $\mathcal U$ of subsets of $2^\omega$ such that if $U_1,\ldots,U_n\in\mathcal U$ then $A_1\cap\cdots\cap A_n\neq\emptyset$, and such that any meager set is disjoint from some set in $\mathcal U$.
\end{prop}
...
\begin{exerc}
  ...
  Prove Proposition~\ref{prop:self-supporting}. [maybe add hint]
\end{exerc}

\begin{exerc}
  \label{exerc:pi-base-meager}
  Prove Proposition~\ref{prop:pi-base-meager}.  [maybe add hint]
\end{exerc}
