...
\usepackage{tikz}
\usetikzlibrary{positioning,decorations.fractals,decorations.pathmorphing}
\usepackage{hyperref}
...
For the rest of this section we introduce the basic combinatorial objects used in forcing. The main object is very simple: a \emph{forcing notion} is simply a partial order $\mathbb P$ with a maximum element $\mathbb1$ (also written $\mathbb1_{\mathbb P}$). When we force with $\mathbb P$, we add a new subset of $\mathbb P$ to the universe. We think of the elements of $\mathbb P$ as imposing conditions on the new set to be added. If $p,q\in\mathbb P$ we say that $p$ is \emph{stronger} than $q$ if $p\leq q$; lower elements hold more information.

When we force with $\mathbb P$, the partial order $\mathbb P$ can give fine control over the new subset that we add.

\begin{defn}
  A subset $G\subset\mathbb P$ is a \emph{filter} if it satisfies the two properties:
  \begin{itemize}
...
\begin{defn}
  A subset $D\subset\mathbb P$ is \emph{dense} if for all $p\in\mathbb P$ there exists $q\in D$ such that $q\leq D$.
\end{defn}

The use of the word ``dense'' here may seem in conflict  with the use of the word in topology, but the two can be connected. If $\mathbb P$ is a partial order then it has a topology with a basis consisting of the downward cones $V_p=\set{q\in\mathbb P\mid q\leq p}$. A subset $D\subset\mathbb P$ is dense if and only if it is dense in this topology.

We think of the dense subsets of $\mathbb P$ as omnipresent---almost every element of $\mathbb P$ lies below some element of $D$. When we force with $\mathbb P$ we add a filter $G$ that is \emph{generic} in the sense that it meets these dense subsets of $\mathbb P$. The more dense sets that we demand that $G$ meets, the stronger a condition this imposes on $G$.

For example, if just countably many dense sets are given, then we will see in the next section that there is always a filter that meets them all. On the other extreme, we could say that a filter is fully generic if it meets \emph{every} dense set of $\mathbb P$, but the next result shows that this is too strong.

\begin{prop}
  Suppose that for every $p\in\mathbb P$, there exist incompatible elements $q,r\leq p$. Then $\mathbb P$ does not have a fully generic filter.
\end{prop}

\begin{proof}
  It is enough to show that if $G\subset\mathbb P$ is a filter then its complement $D=\mathbb P\smallsetminus G$ is dense. For this, let $p\in\mathbb P$ be arbitrary and by the hypothesis on $\mathbb P$ let $q,r$ be incompatible elements such that $q,r\leq p$. Since $G$ is a filter, it cannot contain both $q$ and $r$. Thus $D$ contains at least one of them, and this shows $D$ is dense.
\end{proof}

A partial order $\mathbb P$ satisfying the hypothesis of the proposition is called \emph{atomless}. The condition ensures that forcing with $\mathbb P$ is nontrivial. Once we acknowledge that fully generic filters do not exist in the given universe of set theory, we arrive at the following definition.

\begin{defn}
  If $V$ is a model of set theory and $\mathbb P\in V$, we say that $G$ is \emph{$V$-generic} if for every $D\in V$ such that $D\subset\mathbb P$ is dense, we have $G\cap D\neq\emptyset$.
\end{defn}

When we make forcing rigorous, we will show that under appropriate hypotheses, a $V$-generic filter $G$ can be found outside of $V$. We will then let $W=V[G]$ be the smallest model containing both $V$ and $G$, and call this the forcing extension with respect to the notion $\mathbb P$.

\begin{example}
  Returning to the example of the forcing notion $\mathbb P=\omega^{<\omega}$ with the upside-down ordering, we have already said that a filter $G\subset\omega^\omega$ is actually a path through $\omega^\omega$. If $G$ is $V$-generic, then much more can be said of this path. First of all, consider the sets
...
There is unfortunately one major limitation on the variety of partial orders we will be able to explore using Martin's axiom. A subset $A\subset\mathbb P$ is said to be an \emph{antichain} if for all $p,q\in A$ we have $p$ and $q$ are not compatible. Thus if $A$ is an antichain, a filter $G$ can only choose at most \emph{one} of the elements of $A$.

\begin{defn}
  A partial order $\mathbb P$ is said to be \emph{ccc} (or satisfy the \emph{countable chain condition}) if every antichain $A\subset\mathbb P$ is countable.
\end{defn}

In some sense the ccc imposes a limitation on the number of different filters that may be added when forcing with $\mathbb P$. The partial order $\omega^{<\omega}$ considered in the previous section is obviously ccc because it is countable. However, there are many examples of ccc partial orders which are not countable.

\begin{example}
  \label{ex:cohen-ccc}
  Consider the partial order $\mathbb P$ consisting of all nonempty open subsets of $\RR$, with the ordering $A\leq B$ iff $A\subset B$. Then $\mathbb P$ is ccc: indeed, two sets are incompatible in this ordering if and only if they are disjoint. Thus an antichain is a family of pairwise disjoint open sets, and since $\RR$ is separable, such a family must be countable.
\end{example}

\begin{example}
  Consider the partial order $\mathbb P=\omega_1^{<\omega}$ with the usual upside-down tree ordering. Then this $\mathbb P$ is not ccc. Indeed, the set of sequences $s\in\mathbb P$ of length $1$ is an uncountable antichain in $\mathbb P$.
\end{example}

\begin{defn}
  \emph{Martin's axiom} (or just \emph{MA}) is the statement: For every ccc partial order $\mathbb P$ and every family $\mathcal F$ of fewer than continuum many dense subsets of $\mathbb P$ there exists an $\mathcal F$-generic filter $G\subset\mathbb P$.
\end{defn}
...
\begin{proof}
  Let $\mathbb P$ be any partial order and let $D_0,D_1,\ldots$ be a countable family of dense subsets of $\mathbb P$. Fix $p_0\in D_0$, and inductively let $p_{n+1}\leq p_n$ be an element of $D_{n+1}$. Then the set $\{p_n\}$ is downwards directed, and hence closing it upwards generates a filter $G$ which meets each of the $D_i$.
\end{proof}

In fact, in this proof we did not even need $\mathbb P$ to be ccc. The next result states that MA is also consistent with a larger continuum. For now we take this fact for granted and use it to derive several applications, as an introduction to the combinatorics of forcing. 
...
\begin{prop}
  In the definition of MA, we cannot in general drop the condition that $\mathbb P$ is ccc.
\end{prop}

\begin{proof}
  This time we let use the partial order $\mathbb P=\omega_1^{<\omega}$ with the upside-down order. We have already seen that $\mathbb P$ is not ccc. Now we consider the sets
...
\begin{proof}
  Suppose towards a contradiction that $\mathcal F$ is a family of nowhere dense subsets of $\RR$ of cardinality less than continuum, and that $\bigcup\mathcal F=\RR$. Let $\mathbb P$ be the partial order consisting of all bounded, positive-length intervals in $\RR$, ordered by $I'\leq I$ iff $I'\subset I$. Then $\mathbb P$ is easily seen to be ccc by the same argument as in Example~\ref{ex:cohen-ccc}. For each $A\in\mathcal F$, we consider the set
  \[D_A=\set{I\in\mathbb P\mid \overline{I}\cap A=\emptyset}
  \]
...
%   Thus by MA, there exists a filter $G\subset\mathbb P$ that meets the sets $D_{y,P,n}$ for all $(y,P)\in\mathcal F$, and we let $g=\bigcup G$ be the associated element of $2^\omega$. Given any $(y,P)\in\mathcal F$, since $G$ meets all dense sets $D_{y,P,n}$ it follows that $g$ does not strongly differ from $y$ on $P$. This contradicts our assumption about the family $\mathcal F$.
% \end{proof}
...
\begin{proof}
  Suppose towards a contradiction that $\mathfrak d<\mathfrak c$, and fix a dominating family $\mathcal F\subset\omega^\omega$. Let $\mathbb P$ be the partial order $\omega^{<\omega}$ with the upside-down ordering. For each $f\in\mathcal F$ we consider the family sets
...
\begin{proof}
  Suppose towards a contradiction that $\mathcal F$ is an unbounded family of size $<\mathfrak c$, and let $\mathbb P$ be the Hechler forcing $\mathbb H_{\mathcal F}$. We now consider two families of dense sets. First, for each $n$ we consider the set
...
So far we have seen forcing only in the context of appeals to Martin's axiom. Now we will see how go outside a given model of set theory $V$ to obtain forcing extensions $V[G]$.

Well, that's the idea anyway. In fact, to find ``new'' sets $G$ we will really assume that $V$ is a countable, transitive model ZFC. In other words $V$ is a countable set lying in some larger universe $W$, $V$ it is closed downwards under $\in$, and the structure $(V,\in)$ satisfies all of the axioms of set theory. The payoff is this: since $V$ is countable, given a partial order $\mathbb P\in V$ it is easy to find a filter $G\subset\mathbb P$ that meets the dense subsets of $\mathbb P$ which lie in $V$. We will then form the smallest model $V[G]$ of ZFC which contains all the elements of $V$ together with the element $G$.

The assumption that there is a countable transitive model of ZFC is unfortunately a nontrivial one. The countability isn't the problem; recall that by the L\"owenheim--Skolem theorem if there is an infinite model of a theory $T$ then there is a countable one. Rather, the problem is assuming there is a model of ZFC at all. Indeed this amounts to assuming $\Con(ZFC)$, and we know from G\"odel's incompleteness theorem that this is a nontrivial assumption.

It is possible to work around this technicality by supposing only that $V$ satisfies a large but finite subsystem of ZFC. By the reflection theorem, it is possible to obtain transitive set models of such a subsystem, and it is not difficult to use such a model to get a countable transitive one. However, for simplicity we will simply assume for the rest of this section that
$V$ is a countable, transitive model of all of ZFC.
...
\begin{defn}
  If $\mathbb P$ is a partial order, then a set $\tau$ is called a \emph{$\mathbb P$-name} for an element of $V[G]$ if $\tau$ conists of elements $(\sigma,p)$ where $\sigma$ is a \emph{$\mathbb P$}-name and $p\in\mathbb P$.
\end{defn}

Like sets themselves, the definition is recursive. To explain it briefly, we can think of an ordinary set $x$ as a wellfounded tree with $x$ at the root, the elements of $x$ on the first level, the elements of elements of $x$ on the second level, and so on. A $\mathbb P$-name is similar, except the nodes of the tree are also $\mathbb P$-names, and each node is labelled with an element of $\mathbb P$.

If $\tau$ is a $\mathbb P$-name and we do gain access to a generic filter $G$, then we can use $G$ to resolve the name $\tau$ into an honest set as follows. Thinking of $\tau$ as a tree, we include only the nodes labelled with an element $p\in G$.

\begin{defn}
  If $\mathbb P$ is a partial order in $V$, $\tau$ is a $\mathbb P$-name, and $G\subset\mathbb P$ is a filter, then we define $\tau_G$ recursively by
  ...
  \[V[G]=\set{\tau_G\mid\tau\in V\text{ is a $\mathbb P$-name}}
  \]
\end{defn}

The definitions of $\mathbb P$-name and of $\tau_G$ are heavily recursive, but one should expect this from such a general set-theoretic construction. In fact, it is amazing that such a short-winded construction actually works. But it will take us some time to prove everything we need.

\begin{lem}
  If $\mathbb P$ is a partial order and $G$ is a filter, then we have $V\subset V[G]$ and $G\in V[G]$. Moreover, $M[G]$ is the smallest transitive model with these two properties.
\end{lem}

\begin{proof}
  To prove the statements, we need only write down explicit $\mathbb P$-names for the objects in question. First recall that we are always assuming $\mathbb P$ has a maximum element $\mathbb 1$. Thus if $x\in V$ we can write the recursive definition
  ...
  \[\Gamma=\set{(\check p,p)\mid p\in\mathbb P}
  \]
...
\begin{defn}
  ...
  \begin{itemize}
  \item The \emph{forcing language} for $\mathbb P$ consists of the usual logical symbols, $\in$, and the $\mathbb P$-names as constant symbols.
  \item If $\phi$ is a sentence of the forcing language for $\mathbb P$, we say $\phi$ holds in $V[G]$ if it holds with its names $\tau$ replaced by their values $\tau_G$.
  \item If $p\in\mathbb P$ and $\phi$ is a sentence of the forcing language for $\mathbb P$, then we write $p\Vdash\phi$, read $p$ \emph{forces} $\phi$, if $p\in G$ implies $\phi$ holds in $V[G]$.
  \end{itemize}
\end{defn}
