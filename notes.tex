...
\begin{figure}[h]
  \begin{tikzpicture}[->]
    ...
    \node (covn) [above=1.85 of addn] {$\aleph_1$} edge (addn);
    \node (addm) [right=of addn] {$\aleph_1$} edge (addn);
    \node (b) [above=.65 of addm] {$\aleph_1$} edge (addm);
    \node (nonm) [above=1.85 of addm] {$\aleph_1$} edge (covn) edge (b);
    \node (covm) [right=of addm] {$\mathfrak c$} edge (addm);
    \node (d) [above=.8 of covm] {$\mathfrak c$} edge (b) edge (covm);
...
The random forcing partial order can be defined on any of the spaces $\RR$, $2^\omega$, or $\omega^\omega$, but for technical reasons it will be most convenient to define it on $2^\omega$. Recall that the null ideal $\Null$ on $2^\omega$ is defined by analogy with Definition~\ref{defn:null} with the length $\ell$ replaced by the ``coin-tossing'' measure $m$. Specifically, $A\subset2^\omega$ is \emph{null} if for all $\epsilon>0$ there exists a sequence of basic open sets $(V_{s_n})$ such that $A\subset\bigcup V_{s_n}$ and $\sum2^{-|s_n|}<\epsilon$.

\begin{defn}
  The \emph{random forcing} partial order $\Nonnull(2^\omega)$ consists of equivalence classes $[A]$ of nonnull (Borel) subsets $A\subset2^\omega$. Here, two subsets $A$ and $B$ are called \emph{equivalent} iff the symmetric difference $A\triangle B$ is null. The elements of $\Nonnull(2^\omega)$ are ordered by $[A]\leq[B]$ iff $A\smallsetminus B$ is null.
\end{defn}

\begin{rem}
  We could as well consider the partial order consisting of equivalence classes of \emph{nonmeager} subsets of $2^\omega$, where sets are equivalent if their symmetric difference is \emph{meager}. This partial order is forcing equivalent to Cohen forcing $\Fn(\omega,\omega)$!
\end{rem}

Like Cohen forcing, if $G$ is a $V$-generic filter for random forcing then there is a corresponding generic element $g\in2^\omega$. To define the value of $g(n)$ we let:
\[D_n=\set{[A]\mid A\subset\{x:x(n)=0\}\text{ or }A\subset\{x:x(n)=1\}}\text{.}
\]
Then $D_n$ is dense, since any nonnull set has nonnull intersection with either $\{x:x(n)=0\}$ or $\{x:x(n)=1\}$. Hence there exists $[A]\in G\cap D_n$, and we define $g(n)=0$ or $1$ depending on which of the two cases is satisfied. 

Just as Cohen forcing $\Fn(\omega,\omega)$ has a $\kappa$-sized version $\Fn(\kappa,\omega)$, random forcing has a $\kappa$-length version $\Nonnull(2^\kappa)$. Here, the null sets of $2^\kappa$ are defined in exactly the same way as in $2^\omega$, except that the basic open subsets of $2^\kappa$ are of the form $V_s$ where $s$ is a finite partial function from $\kappa$ to $2$.

Like the $\kappa$-sized Cohen forcing, the $\kappa$-sized random forcing adds $\kappa$ many new elements to Cantor space and therefore pushes the value of the continuum up to $\kappa$.
...
\begin{proof}
  ...
  We next claim that $\PP$ adds $\kappa$ many distinct elements to $2^\omega$. 
...
Before the proof we need to explore the effect of the countable-length random forcing on the domination order. The following result should be compared with Lemma~\ref{lem:cohen-dominating}.

\begin{lem}
  Let $\PP=\Nonnull(2^\omega)$ and let $G$ be a $V$-generic filter. Then in $V[G]$ we have that for all $h\in\omega^\omega$ there exists $f\in \omega^\omega\cap V$ such that $h\leq f$.
\end{lem}
...
\begin{proof}[Proof of Theorem~\ref{thm:random-bd}]

\end{proof}

\begin{exerc}
  Show that the random forcing partial order, together with $[\emptyset]$, forms a complete Boolean algebra. [def]
\end{exerc}
...
Just as Cohen reals lie in every comeager set coded in the ground model, random reals lie in every conull set coded in the ground model.  As a consequence, the effect of Cohen forcing on the characteristics of $\Meager$ is analogous to the effect random forcing on the characteristics of $\Null$.
