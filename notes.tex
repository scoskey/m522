...
In the previous sections we studied the effect of the forcing $\Fn(\kappa,\omega)$ on the value of the continuum. Next, we study the effect of the forcing with $\Fn(\kappa,\omega)$ on the value of the other cardinal characteristics in Cicho\'n's diagram. In this section, we study only the effect of Cohen forcing on $\mathfrak b$ and $\mathfrak d$.

For this, we first study the effect of adding just one element to the Baire space. The next result showcases the interesting role that Cohen reals play with respect to the domination order $\leq^*$.

\begin{lem}
  \label{lem:cohen-dominating}
  Let $\PP=\Fn(\omega,\omega)$, and let $G$ be a $V$-generic filter. Then we have:
  \begin{itemize}
  \item If $g=\bigcup G$, then $g$ is not dominated by any element of $V$.
  \item If $h\in V[G]$, then $h$ does not dominate every element of $V$.
  \end{itemize}
\end{lem}
...
\begin{proof}
  ...
  \[h_i(n)=\max\set{m\mid p_i\forces\check m\leq\dot h(\check n)}\text{.}
  \]
  (If $p_i$ does not even force that $h(n)$ is an integer, then set $h_i(n)=0$ by default.) Now the $h_i$ are defined in $V$, so in $V$ we can diagonalize against them to define the function
  \[f(n)=\max\set{h_0(n),\ldots,h_n(n)}+1\text{.}
  \]

  We aim to show that this $f$ is in fact not dominated by $h$. If it were, there would be some $p\in\PP$ such that $p\forces \check f\leq^*\dot h$. In detail, this means that $p\forces(\exists N)(\forall n\geq N)\;\check f(n)\leq\dot h(n)$. By Lemma~\ref{lem:forcing-exists}, there exists $i$ and $N$ such that $p_i\forces (\forall n\geq\check N)\;\check f(n)\leq\dot h(n)$. 
...
\begin{proof}
  ...
  To show that $G_1$ is $V$-generic for $\PP_1$, \ldots

  To show that $G_2$ is $V[G_1]$-generic for $\PP_2$ takes a little more work. For this, let $D\in V[G_1]$ be a dense subset of $\PP_2$, and let $\dot D$ be a $\PP_1$-name for $D$. Using the forcing lemma, we can find $p_1\in G_1$ such that $p_1\forces$ ``$\dot D$ is dense in $\check\PP_2$''. Let
...
\begin{rem}
  The second claim of the theorem improves the result of Theorem~\ref{thm:ch-exact}, which showed that $\Fn(\kappa,\omega)$ enlarges $\mathfrak c$ d to $\kappa$.
\end{rem}
