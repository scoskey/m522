...
In the previous sections we studied the effect of the forcing $\Fn(\kappa,\omega)$ on the value of the continuum. Next, we study the effect of the forcing $\Fn(\kappa,\omega)$ on the value of the other cardinal characteristics in Cicho\'n's diagram.
...
We leave the combinatorial proof of this elementary result to the reader. [REF Kunen, Theorem~V.2.18]
...
\begin{proof}
  ...
  \[p\forces (\forall f\in\omega^\omega\cap V)\;f\leq^*h\text{.}
  \]
  Since $\PP_1$ is countable, we can enumerate the elements $q\leq p$ in a list $\{q_i\}$. For each $i$ and $n$, we consider the set
  \[S_{i,n}=\set{\check m\mid q_i\forces\check m\leq\dot h(\check n)}\text{.}
  \]
  Observe that since $q_i\forces h(n)\in\omega$, for each $i$ and $n$ the set $S_{i,n}$ is finite. Therefore we can diagonalize against these sets to define the function $f(n)=\max(S_{0,n}\cup\cdots\cup S_{n,n})+1$.
  ...
  Finally, letting $n\geq N,i$ and $m=f(n)$, we have $q_i\forces\check m\leq\dot h(\check n)$. It follows that $m\in S_{i,n}$, and by definition of $f$ this means we have let $f(n)>m$. This is a contradiction!
\end{proof}
