...
\begin{rem}
  The well-order property may seem obscure at first, but finding a least element is precisely what is needed in induction arguments (think: ``if not, let $x$ be the least counterexample'').
\end{rem}

In the next few results, we essentially show that the cardinalities are well-ordered. Note that reflexivity holds because the identity function is an injection from $A$ into itself, and transitivity holds because the composition of two injections is an injection. The following result establishes antisymmetry.
...
\begin{thm}
  \label{thm:card-well}
  Let $\mathcal A$ be a family of sets. Then there exists $A\in\mathcal A$ such that for all $B\in\mathcal A$ we have $|A|\leq|B|$.
\end{thm}

By applying this result to the family $\mathcal F=\{A_0,A_1\}$, we conclude that either $|A_0|\leq|A_1|$ or $|A_1|\leq|A_2|$, and hence the total ordering property holds. In the proof we will need the following consequence of the axiom of choice.

\begin{thm}[Zorn's lemma]
  If $P$ is a partially ordered set such that every totally ordered subset has an upper bound, then $P$ has at least one maximal element.\qed
\end{thm}

\begin{proof}[Proof of Theorem~\ref{thm:card-well}]
  We give the proof first in the case when $\mathcal A$ is a \emph{set}. (This will be needed to apply Zorn's lemma; note that technically $\mathcal A$ can also be a ``proper class'' such as the set of all sets.) Let $A$ be any fixed element of $\mathcal A$ and consider the set
\[\mathcal F=\set{(f_B)_{B\in\mathcal F}\mid \text{there is $A_0\subset A$ such that for all $B$, $f_B$ is an injection from $A_0$ to $B$}}
\]
and partially order $\mathcal F$ by coordinatewise extension of functions. It is easy to verify that this partial order satisfies the conditions of Zorn's lemma, so there exists a maximal element $(f_B)_{B\in\mathcal F}$. Maximality implies one of two things: either the domain $A_0$ cannot be extended, or the range of one of the functions $f_B$ cannot be extended.

In the first case we have $A_0=A$ and hence $f_B$ is an injection from $A$ to $B$ for all $B$, so $A$ is the minimal set we seek. In the second case there is $B$ such that $f_B(A_0)=B$. It follows that for all $D\in\mathcal F$, the composition $f_D\circ f_B^{-1}$ is an injection from $B$ to $D$, so $B$ is the minimal set. This completes the proof in the case when $\mathcal A$ is a set.

In the general case that $\mathcal A$ is a class, we can reduce to the set case as follows. Fix an element $A\in\mathcal A$ and let $\mathcal A'$ denote the collection of subsets of $A$ that are in bijection with some subset of $\mathcal A$. Then $\mathcal A'$ is a set, and so we may let $A_0$ be of minimal cardinality among all elements of $\mathcal A'$. Now if $A_1\in\mathcal A$ is in bijection with $A_0$, then $A_1$ is of minimal cardinality among all elements of $\mathcal A$.
\end{proof}
...
Ordinal numbers play a central role in set theory, including both cardinality theory and forcing. While cardinal numbers are needed to measure the \emph{size} of an infinite set, ordinal numbers are needed to measure the \emph{length} of an infinite well-ordered set. In this section we introduce the ordinals and state many of the most important facts about them. We will also use the ordinals to give the official definition of the $\aleph$ numbers defined in the previous section.

The goal in defining the ordinals is to provide a collection of well-ordered sets such that for any given well-ordered set $A$ there is one and only one ordinal $\alpha$ such that $A$ is isomorphic to $\alpha$. For finite well-ordered sets, we can write such a definition explicitly:
\begin{align*}
0&=\emptyset\\
1&=\{0\}\\
2&=\{0,1\}\\
...
n+1&=\{0,\ldots,n\}
\end{align*}
This construction can be summed up in one recurrence: $n+1=n\cup\{n\}$. The first infinite ordinal, called $\omega$ or $\omega_0$, is simply the union of all the finite ordinals.

Intuitively, infinite ordinals are constructed similarly. That is, if $\alpha$ is an ordinal then the next one is $\alpha+1=\alpha\cup\{\alpha\}$. And after any infinite sequence of ordinals, we will have $\lambda&=\bigcup_{\alpha<\lambda}\alpha$. Unfortunately, this last formula doesn't work as a definition because it is circular. Instead we have the following more technical characterization.

\begin{defn}
  A set $\alpha$ is an \emph{ordinal} if it satisfies the properties:
  \begin{itemize}
  \item $\alpha$ is transitive: if $x\in\alpha$ and $y\in x$ then $x\in\alpha$;
  \item $\alpha$ is well-ordered by the element relation $\in$
  \end{itemize}
\end{defn}

\begin{rem}
The second condition ensures that the ordinals are well-ordered sets, and in fact that the order relation is the most convenient one available in set theory. The first condition ensures that the ordinals have no ``gaps''; for instance the set 
\end{rem}

\begin{thm}
  If $A$ is a well-ordered set then there exists a unique ordinal $\alpha$ such that $A$ is isomorphic to $\alpha$.
\end{thm}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The domination order on Cantor and Baire space}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
