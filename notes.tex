...
\begin{proof}
  ...
  To see that there is no bijection between $A$ and $\mathcal P(A)$, let $f$ be any function $f\colon A\to\mathcal P(A)$. Then build the set $D$ of all elements $a\in A$ such that $a\notin f(a)$. We claim that $D$ is not in the range of $f$, and therefore that $f$ is not a bijection.
...
\begin{rem}
  The classical argument above is given the name ``diagonal'' because of the key formula in the proof: $a\notin f(a)$. The idea is that if you were to shade the set of pairs $(x,y)\in A^2$ such that that $x\notin f(y)$, then the set $D$ would be built by taking the unshaded elements of the diagonal of the $A^2$ plane.
\end{rem}
...
\begin{proof}[Proof of Theorem~\ref{thm:card-well}]
  Let $A$ be any fixed element of $\mathcal A$ and consider the set
...
In the first case we have $A_0=A$ and hence $f_B$ is an injection from $A$ to $B$ for all $B$, so $A$ is the minimal set we seek. In the second case there is $B$ such that $f_B(A_0)=B$. It follows that for all $D\in\mathcal F$, the composition $f_D\circ f_B^{-1}$ is an injection from $B$ to $D$, so $B$ is the minimal set.
\end{proof}
...
In this section we introduce the ordinals, which play a central role in set theory, including both cardinality theory and forcing. While cardinal numbers measure the \emph{size} of a set, ordinal numbers measure the \emph{position} of an element in an enumeration. Ordinals also permit us to give a more rigorous and more satisfying definition of the $\aleph$ numbers than we did in the previous section.

The ordinals are a class of well-ordered sets, and for each well-ordered set there is one and only one ordinal isomorphic to it. For finite well-ordered sets, here is the official way to do this:
\begin{align*}
0&=\emptyset\\
1&=\{0\}\\
2&=\{0,1\}\\
...
n+1&=\{0,\ldots,n\}
\end{align*}
The single rule holding these together is $n+1=n\cup\{n\}$. After infinitely many ordinals have constructed, the next ordinal is defined as the union of all those that came before it. Thus we obtain the rules
\begin{align*}
\alpha+1&=\alpha\cup\{\alpha\}\\
\lambda&=\bigcup_{\alpha<\lambda}\alpha
\end{align*}
where the second rule only holds if the set of $\alpha<\lambda$ have no last element.
