...
\begin{proof}
  First suppose that $p\forces\neg\phi$ and let $q\leq p$. Then by Lemma~\ref{lem:forcing-downward} we have $q\forces\neg\phi$. Then we cannot have $q\forces\phi$, since then if $G$ is a $V$-generic filter with $q\in G$ we would have $V[G]$ satisfies both $\phi$ and $\neg\phi$.
...
The proof of the forcing theorems now proceeds in a series of lemmas. Our first lemma states that the analog of Lemma~\ref{lem:forcing-downward} holds for $\forces_0$. This simple fact will be used throughout the rest of the proof.
