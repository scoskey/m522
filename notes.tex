...
If $\tau$ is a $\PP$-name and we do gain access to a generic filter $G$, then we can use $G$ to evaluate the name $\tau$ as follows: Again thinking of $\tau$ as a tree, we include only the nodes whose label $p$ happens to lie in $G$.
...
The remaining axioms of replacement, power set, and choice are also existence axioms, and they can once again be proved by coming up with clever names. As in this argument, the names are defined using the forcing relation.
