...
\begin{proof}
  We begin by writing $A=\bigcup A_n$ where each $A_n$ is nowhere dense. As usual, replacing $A_n$ by its closure if necessary we may assume that each $A_n$ is closed. Moreover, taking successive unions we may assume that $A_n\subset A_{n+1}$. We define the chopped real $(x,\Pi)$ recursively. To begin, since $A_0$ isn't dense there exists $s\in2^{<\omega}$ such that $A_0\cap V_s=\emptyset$. We let $\Pi_0=\dom(s)$ and $x\restriction\Pi_0=s$.

  Now suppose that $\Pi_i$ and $x\restriction\Pi_i$ have been constructed for $i<n$ and let $s=x\restriction\bigcup_{i<n}\Pi_i$ be the segment built so far. Since $A_n$ is nowhere dense, we can find an extension $t$ of $s$ such that $A_n\cap V_t=\emptyset$. But we can do even better: let $s_0,\ldots,s_{k-1}$ enumerate all elements of $2^{<\omega}$ whose domain is exactly $\bigcup_{i<n}\Pi_i$. Then we can actually find an extension $t$ of $s$ such that for all $i<k$ we have $V_{s_i\cup(t\smallsetminus s)}\cap A_n=\emptyset$.

  Such a $t$ is built in $k$ steps. \ldots

  Now we let $\Pi_n=\dom(t\smallsetminus s)$ and $x_n=t\restriction\Pi_n$.

  Finally if $a\in A_n$ and $n'\geq n$ then $a\restriction\Pi_{n'}$
\end{proof}
