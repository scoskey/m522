...
\begin{proof}
  Beginning with the implication $(\Leftarrow)$, it is enough to show any set of the form $\Diff_P(x)$ is meager. For this, let
  ...
  For $(\Rightarrow)$, suppose $A=\bigcup A_n$ where each $A_n$ is nowhere dense, and assume without loss of generality that $A_n\subset A_{n+1}$ for all $n$. We will define an interval partition $P=(I_n)$ and an element $x\in2^\omega$ recursively. To begin, since $A_0$ isn't dense there exists $s\in2^{<\omega}$ such that $A_0\cap V_s=\emptyset$. We let $I_0=\dom(s)$ and $x\restriction I_0=s$.
