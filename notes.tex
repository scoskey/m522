...
\begin{exerc}
  Suppose that $g$ is the generic element of $2^\kappa$ added by random forcing. Suppose that for each $n$, $s_n\in2^{<\omega}$ and the sequence $\langle s_n\rangle\in V$. Show that if $\bigcup V_{s_n}$ is conull, then $g\in\bigcup V_{s_n}$.
\end{exerc}
...
\begin{lem}
  Let $\PP=\Nonnull(2^\kappa)$ and let $G$ be $V$-generic. If $A$ is a $G_\delta$ subset of $2^\omega$, then the measure of $A$ is the same whether interpreted in $V$ or $V[G]$.
\end{lem}
