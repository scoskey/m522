...
\begin{rem}
  ...
  It follows from the proof of Theorem~\ref{thm:k-sigma} that every $\mathcal K_\sigma$ set in $\omega^\omega$ is meager. Indeed, compact sets are closed, and moreover since they are bounded they cannot contain any basic open sets $V_s$. It follows that compact sets are nowhere dense, and hence $\sigma$-compact sets are meager. This in turn implies the Baire category property for $\sigma$-compact sets: $\omega^\omega$ is not $\sigma$-compact and any basic open neighborhood $V_s$ is not $\sigma$-compact.
\end{rem}
...
\begin{exerc}
  Find an example of a meager subset of $\omega^\omega$ which is not $\mathcal K_\sigma$.
\end{exerc}
...
\begin{figure}[h]
  \begin{tikzpicture}[->]
    \node (a1) {$\aleph_1$};
    \node (add) [right=of a1] {$\textbf{add}(\mathcal{I})$} edge (a1);
    \node (non) [above=of add] {$\textbf{non}(\mathcal{I})$} edge (add);
    \node (cov)	[right=of add] {$\textbf{cov}(\mathcal{I})$} edge (add);
    \node (cof) [above=of cov] {$\textbf{cof}(\mathcal{I})$}
      edge (non) edge (cov);
...
\begin{figure}[h]
  \begin{tikzpicture}[->]
    \node (a1) {$\aleph_1$};
    \node (addn) [right=of a1] {$\add(\mathcal N)$} edge (a1);
    \node (covn) [above=2 of addn] {$\cov(\mathcal N)$} edge (addn);
    \node (addm) [right=of addn] {$\add(\mathcal M)$} edge (addn);
    \node (b) [above=.8 of addm] {$\mathfrak b$} edge (addm);
    \node (nonm) [above=2 of addm] {$\non(\mathcal M)$} edge (covn) edge (b);
    \node (covm) [right=of addm] {$\cov(\mathcal M)$} edge (addm);
    \node (d) [above=.8 of covm] {$\mathfrak d$} edge (b) edge (covm);
    \node (cofm) [above=2 of covm] {$\cof(\mathcal M)$} edge (nonm) edge (d);
    \node (nonn) [right=of covm] {$\non(\mathcal N)$} edge (covm);
    \node (cofn) [above=2 of nonn]{$\cof(\mathcal N)$} edge (cofm) edge (nonn);
...
\begin{rem}
  By Exercise~\ref{exerc:meager-bij}, the symbol $\mathcal M$ can stand for the meager sets on any one of the spaces $\RR$, $2^\omega$, or $\omega^\omega$. Similarly, in Exercise~\ref{exerc:null-bij} we give a natural measure on $2^\omega$, and again show that $\mathcal N$ can stand for the null sets on either $\RR$ or $2^\omega$. (We omit the definition of an appropriate measure on $\omega^\omega$ [ref].) % blass
\end{rem}
...
For the rest of this section as well as the next few, we take up the proof of Theorem~\ref{thm:cichon}. We have already established seven out of the fifteen inequalities depicted: $\mathfrak b\leq\mathfrak d$ follows from Proposition~\ref{prop:bd}. The two bounds $\aleph_1\leq\add(\mathcal N)$ and $\cof(\mathcal N)\leq\mathfrak c$ follow from Lemma~\ref{lem:ideal-cards}. And four more of the inequalities are special cases of Lemma~\ref{lem:diamond}.

We now present four more fairly easy inequaleties, leaving the remaining four for the next two sections.

\begin{lem}
  \label{lem:b-nonm}
  We have $\mathfrak b\leq\non(\mathcal M)$, and $\cov(\mathcal M)\leq\mathfrak d$.
\end{lem}

\begin{proof}
  For the first inequality, recall our observation in Remark~\ref{rem:ksigma-meager} that $\mathcal K_\sigma\subset\mathcal M$, so any nonmeager subset of $\omega^\omega$ is certainly not $\mathcal K_\sigma$. But we also know that every non-$\mathcal K_\sigma$ set is unbounded. It follows that every nonmeager subset of $\omega^\omega$ is unbounded, which implies $\mathfrak b\leq\non(\mathcal M)$.

  The second inequality again uses the fact that $\mathcal K_\sigma\subset\mathcal M$, and we leave it as Exercise~\ref{exerc:covm-d}.
\end{proof}

\begin{lem}
  \label{lem:covn-nonm}
  We have $\cov(\mathcal N)\leq\non(\mathcal M)$, and $\cov(\mathcal M)\leq\non(\mathcal M)$.
\end{lem}
...
\begin{exerc}
  \label{exerc:covm-d}
  Show that $\cov(\mathcal M)\leq\mathfrak d$.
\end{exerc}
...
\]
such that for all $c\in\dom(S)$ and $d\in\cod(R)$ we have
...
\end{tikzpicture}
\end{center}
In any case, the definition of a morphism is custom-designed to give us the following result.

\begin{prop}
  If $(\phi,\psi)$ is a morphism from $R$ to $S$, then $\mathfrak d(R)\geq\mathfrak d(S)$.
\end{prop}

\begin{proof}
  It is enough to show that if $\mathcal F$ is an $R$-dominating family, then the image $\psi(\mathcal F)$ is an $S$-dominating family, since this image has the same or smaller cardinality than $\mathcal F$. To see this, if $d\in\dom(S)$ then since $\mathcal F$ is $R$-dominating we know there is $c\in\mathcal F$ such that $\phi(d)\mathrel{R}c$. Since $(\phi,\psi)$ is a morphism, we then know that $d\mathrel{S}\psi(c)$. This confirms that $\psi(\mathcal F)$ is an $S$-dominating family.
\end{proof}

For most of the inequalities between cardinal characteristics that we have seen, it is possible to extract a morphism hiding in the proof.

\begin{example}
  In our proof that $\cov(\mathcal I)\leq\cof(\mathcal I)$, we could have used the morphism from $\subset$ relation on $\mathcal I$ to the $\in$ relation on $X\times\mathcal I$ given by the maps
\[\begin{cases}\phi(x)=\{x\}\\\psi(B)=B\end{cases}
\]
Indeed, it is trivial to see that $\phi(x)\subset B\implies x\in\psi(B)$.
\end{example}

\begin{example}
  In Lemma~\ref{lem:covn-nonm}, where we showed that $\cov(\mathcal N)\leq\non(\mathcal M)$, we could have used the following morphism from the $\not\ni$ relation on $\mathcal M\times2^\omega$ to the $\in$ relation on $2^\omega\times\mathcal N$. Again we let $A$ be a fixed null comeager set.
\[\begin{cases}\phi(x)=x-A^c\\\psi(x)=x+A\end{cases}
\]
This time, we have that $\phi(x)\not\ni y$ implies $x-y\in A$, which in turn implies that $x\in y+A=\psi(y)$, as required.
\end{example}
...
An \emph{interval partition} is a partition $P=(I_n)$ of $\omega$ into finite nonempty intervals $I_n$. We always assume that the $I_n$ is adjacent to $I_{n+1}$, and hence also that they are enumerated in increasing order.

\begin{defn}
  \begin{itemize}
  \item A \emph{chopped real} is a pair $(x,P)$ where $x\in2^\omega$ and $P$ is an interval partition.

  \item If $(x,P)$ is a chopped real, we say $y\in2^\omega$ \emph{strongly differs} from $(x,P)$ if for all but finitely many $n$ we have $y\restriction I_n\neq x\restriction I_n$.
  \end{itemize}
\end{defn}

The idea behind the chopped real terminology is that the real number $x$ has been chopped into countably many finite pieces $x\restriction I_n$. If $(x,P)$ is a chopped real, we define the set $\Diff(x,P)$ to be the set of all $y\in2^\omega$ such that $y$ strongly differs from $(x,P)$. The following result gives a new, combinatorial basis for the meager ideal on Cantor space.

\begin{lem}
  A subset $A\subset2^\omega$ is meager if and only if there is some chopped real $(x,P)$ such that $A\subset\Diff(x,P)$.
\end{lem}

\begin{proof}
  We begin by writing $A=\bigcup A_n$ where each $A_n$ is nowhere dense and $A_n\subset A_{n+1}$ for all $n$. We will define the chopped real $(x,P)$ recursively. To begin, since $A_0$ isn't dense there exists $s\in2^{<\omega}$ such that $A_0\cap V_s=\emptyset$. We let $I_0=\dom(s)$ and $x\restriction I_0=s$.

  Now suppose that $I_i$ and $x\restriction I_i$ have been constructed for $i<n$ and let $s=x\restriction\bigcup_{i<n}I_i$ be the segment built so far. Since $A_n$ is nowhere dense, by Proposition~\ref{prop:cantor-space-nwd} we can find an extension $t$ of $s$ such that $A_n\cap V_t=\emptyset$. But we can do even better: we can find an extension $t$ of $s$ such that even if $y\in2^\omega$ only agrees with $t$ on $\dom(t)\smallsetminus\dom(s)$, then we still have $y\notin A_n$.

  Such a $t$ is built in $k$ many steps: Let $s_0,\ldots,s_{k-1}$ enumerate all elements of $2^{<\omega}$ whose domain is exactly $\dom(s)$. Repeatedly using that $A_n$ is nowhere dense, we can find a sequence of $t_0,t_1,\ldots,t_{k-1}=t$  such that $A_n\cap V_{s_i\cup(t_i\smallsetminus s)}=\emptyset$. Then we let $I_n=\dom(t\smallsetminus s)$ and $x_n=t\restriction I_n$. This concludes the construction.

  To see that $A\subset\Diff(x,P)$, observe that whenever $y\restriction I_n=x\restriction I_n$ we have $y\notin A_n$. In other words, whenever $y\in A_n$ we have that $y$ differs from $x$ on $I_n$. Since the $A_n$ are increasing, any $a\in A$ lies in all but finitely many of the $A_n$. Thus we can conclude that any $a\in A$ strongly differs from $(x,P)$.
\end{proof}

The lemma implies that the collection of sets $\Diff(x,P)$ form a basis for the meager ideal on Cantor space.

\begin{lem}
  We have $\add(\mathcal M)\leq\mathfrak b$.
\end{lem}
