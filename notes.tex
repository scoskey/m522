...
At the end of the section we will describe exactly what the $\aleph$'s are. But first, we give some of the most important facts about cardinality.
...
\]
Therefore, $A$ can be written as a union
\[(A\mathord{\smallsetminus}B)\cup(B\mathord{\smallsetminus}i(A))\cup(i(A)\mathord{\smallsetminus}i(B))\cup\cdots\cup \bigcap i^n(A)
\]
Moreover, $i$ gives bijections
\[A\smallsetminus B\leftrightarrow i(A)\smallsetminus i(B)\leftrightarrow i^2(A)\smallsetminus i^2(B)\leftrightarrow\cdots
\]
It also gives bijections
\[B\smallsetminus i(A)\leftrightarrow i(B)\smallsetminus i^2(A)\leftrightarrow i^2(B)\smallsetminus i^3(A)\leftrightarrow
\]
...
We conclude by briefly diving further into set theory and giving a more rigorous definition of the $\aleph$ numbers. But first it is necessary to define the ordinals, which appear as the subscripts $\aleph_\alpha$. While cardinals like $\aleph_3$ measure the \emph{size} of a set, ordinals like $\alpha$ measure the \emph{position} in an enumeration.
