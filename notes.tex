...
\usepackage{tikz}
\usetikzlibrary{positioning,decorations.fractals}
...
\begin{prop}
  ...
  \begin{itemize}
  \item $\cof(\mathcal I)$ is the dominating number of the subset relation $\subset$ on $\mathcal I$.
  \item $\cov(\mathcal I)$ is the dominating number of the $\in$ relation from $X$ to $\mathcal I$.
  \item $\non(\mathcal I)$ is the dominating number of the $\not\ni$ relation from $\mathcal I$ to $X$.
...
So far we have only described the objects of our category, namely the binary relations $R$. Next we describe the morphsims.

\begin{defn}
  Let $R\subset A\times B$ and $S\subset C\times D$ be relations. A \emph{morphism} from $R$ to $S$ is a pair of maps
\[\begin{cases}\phi\colon C\to A\\\psi\colon B\to D\end{cases}
\]
such that for all $c\in C$ and $b\in B$ we have
\[\phi(c)\mathrel{R}b\iff c\mathrel{S}\psi(b)
\]
\end{defn}

\begin{prop}
  If there is a morphism from $R$ to $S$, then $\mathfrak d(R)\geq\mathfrak d(S)$.
\end{prop}
...
An \emph{interval partition} is a partition $\Pi=(\Pi_n)$ of $\omega$ into finite nonempty intervals $\Pi_n$. We always assume that the $\Pi_n$ are enumerated in increasing order.

\begin{defn}
  \begin{itemize}
  \item A \emph{chopped real} is a pair $(x,\Pi)$ where $x\in2^\omega$ and $\Pi$ is an interval partition.

  \item If $(x,\Pi)$ is a chopped real, we say $y\in2^\omega$ \emph{strongly differs} from $(x,\Pi)$ if for all but finitely many $n$ we have $y\restriction\Pi_n\neq x\restriction\Pi_n$.
  \end{itemize}
\end{defn}

The idea behind the chopped real terminology is that the real number $x$ has been chopped into countably many finite pieces $x\restriction\Pi_n$. If $(x,\Pi)$ is a chopped real, we define the set $\Diff(x,\Pi)$ to be the set of all $y\in2^\omega$ such that $y$ strongly differs from $(x,\Pi)$. The following result gives a new, combinatorial basis for the meager ideal on Cantor space.

\begin{lem}
  A subset $A\subset2^\omega$ is meager if and only if there is some chopped real $(x,\Pi)$ such that $A\subset\Diff(x,\Pi)$.
\end{lem}

\begin{proof}
  We begin by writing $A=\bigcup A_n$ where each $A_n$ is nowhere dense and $A_n\subset A_{n+1}$ for all $n$. We will define the chopped real $(x,\Pi)$ recursively. To begin, since $A_0$ isn't dense there exists $s\in2^{<\omega}$ such that $A_0\cap V_s=\emptyset$. We let $\Pi_0=\dom(s)$ and $x\restriction\Pi_0=s$.

  Now suppose that $\Pi_i$ and $x\restriction\Pi_i$ have been constructed for $i<n$ and let $s=x\restriction\bigcup_{i<n}\Pi_i$ be the segment built so far. Since $A_n$ is nowhere dense, we know we can find an extension $t$ of $s$ such that $A_n\cap V_t=\emptyset$. But we can do even better: we can find an extension $t$ of $s$ such that even if $y\in2^\omega$ only agrees with $t$ on $\dom(t)\smallsetminus\dom(s)$, then we still have $y\notin A_n$.

  Such a $t$ is built in $k$ many steps: Let $s_0,\ldots,s_{k-1}$ enumerate all elements of $2^{<\omega}$ whose domain is exactly $\dom(s)$. Repeatedly using that $A_n$ is nowhere dense, we can find a sequence of $t_0,t_1,\ldots,t_{k-1}=t$  such that $A_n\cap V_{s_i\cup(t_i\smallsetminus s)}=\emptyset$. Then we let $\Pi_n=\dom(t\smallsetminus s)$ and $x_n=t\restriction\Pi_n$. This concludes the construction.

  To see that $A\subset\Diff(x,\Pi)$, observe that whenever $y\restriction\Pi_n=x\restriction\Pi_n$ we have $y\notin A_n$. In other words, whenever $y\in A_n$ we have that $y$ differs from $x$ on $\Pi_n$. Since the $A_n$ are increasing, any $a\in A$ lies in all but finitely many of the $A_n$. Thus we can conclude that any $a\in A$ strongly differs from $(x,\Pi)$.
\end{proof}

The lemma implies that the collection of sets $\Diff(x,\Pi)$ form a basis for the meager ideal on Cantor space.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Bartoszynski's theorem}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
