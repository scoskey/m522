...
If $x\in2^\omega$ is fixed and $P$ is an interval partition, we define the set $\Diff(x,P)$ to be the set of all $y\in2^\omega$ such that $y$ strongly differs from $x$ on $P$. Since an element $x\in2^\omega$ and an interval partition $P$ appear together very frequently, such a pair $(x,P)$ has been given the name \emph{chopped real}. The following result shows that the chopped reals give us a new, combinatorial basis for the meager ideal on Cantor space.

\begin{lem}
  A subset $A\subset2^\omega$ is meager if and only if there is some $x\in2^\omega$ and interval partition $P$ such that $A\subset\Diff(x,P)$.
\end{lem}
...
The lemma implies that the collection of sets $\Diff(x,P)$ forms a basis for the meager ideal on Cantor space.
