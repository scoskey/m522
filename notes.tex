...
If $X$ is any set, then an \emph{ideal} on $X$ is a subset $\mathcal I\subset\mathcal P(X)$ which is closed under unions and subsets. An ideal $\mathcal I$ is called a \emph{$\sigma$-ideal} if it is additionally closed under countable unions. We have already discussed three key examples of $\sigma$-ideals.
  \begin{itemize}
...
Now, we define cardinal characteristics related to ideals.
...
\begin{defn} Let $\mathcal{I}$ be an ideal of subsets of a set $X$, such that $\mathcal I$ contains all singletons of $X$. 
  ...
  \item The \emph{cofinality} of $\mathcal{I}$, $\cof(\mathcal{I})$, is the smallest cardinality of any subset $\mathcal{B}$ of $\mathcal{I}$ such that every element of $\mathcal{I}$ is a subset of an element of $\mathcal{B}$.
  \end{itemize}
\end{defn}

Like the cardinals we have seen in the last section, assuming $\mathcal I$ is reasonable, each of these cardinals gives an uncountable lower bound for the value of the continuum.

\begin{lem}
  Assume that $\mathcal I$ is a proper $\sigma$-ideal such that $\mathcal I$ contains the singletons and $\mathcal I$ has a basis of Borel sets. Then each of the four cardinals above is an uncountable lower bound for $\mathfrak c$.
\end{lem}
...
\begin{lem}
  Suppose that $\mathcal I$ is proper a $\sigma$-ideal such that $\mathcal I$ contains the singletons. Then we have $\add(\mathcal I)\leq\non(\mathcal I)\leq\cof(\mathcal I)$, and also $\add(\mathcal I)\leq\cov(\mathcal I)\leq\cof(\mathcal I)$.
\end{lem}

\begin{proof}
  We work from left to right over (3.1), and follow similarly with (3.2).

$(1)$ {Suppose $A\subset \mathcal{I}$ is of cardinality $\kappa$, such that $\cup A$ covers $X$. We need to exhibit a $B\subset \mathcal{I}$ of size $\kappa$, such that $\cup{B}\not\in \mathcal{I}$. Let $B=A$: then we have that $\cup B\supset X$ and since $\mathcal{I}$ is proper, it must be that $\kappa\geq \add(\mathcal{I})$  }\\
...
\begin{figure}[h]
  \begin{tikzpicture}
    ...
    \draw[<-] (Cof) to node {} (C);
  \end{tikzpicture}
  \caption{The relationships between the cardinal characteristics of $\mathcal I$, assuming $\mathcal I$ satisfies the hypotheses of the two lemmas. In the figure, $\leftarrow$ represents $\leq$.\label{fig:ideal}}
\end{figure}
