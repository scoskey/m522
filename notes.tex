...
It takes a moment to confirm that the above definition isn't circular. For example $\sigma\in\tau$ is defined in terms of equality, which using negation is defined in terms of inequality, which is defined in terms of $\notin$, which is defined in terms of $\in$. The key thing to notice is that when we encounter $\in$ for the second time, it will concern names of \emph{lower complexity}.
