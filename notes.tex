...
Recall G\"odel's incompleteness theorem. The usual axioms of set theory cannot decide the truth of every statement. Any collection of axioms such as ZFC will have many models. The theorems of ZFC are the properties that hold in every model. But there are many statements, such as CH, which hold in some models and fail to hold in others. Such statements are called \emph{independent}.

Forcing is a method of adding new sets to a given model $V$ of ZFC, to obtain a larger model $W$ of ZFC. By carefully choosing \emph{how} we add the new sets, we may be able to \emph{force} a given statement to hold in $W$. For example if $\mathcal F\subset\omega^\omega$, it is possible to add a new element $g\in\omega^\omega$ that is a bound for $\mathcal F$, even if $\mathcal F$ was unbounded to begin with. This is a powerful move, and when iterated it can have an effect on the value of the cardinal $\mathfrak b$.
...
We think of the dense subsets of $\mathbb P$ as omnipresent---almost every element of $\mathbb P$ lies below some element of $D$. When we force with $\mathbb P$ we add a filter $G$ that is \emph{generic} in the sense that it meets these dense subsets of $\mathbb P$. The more dense sets that we demand that $G$ meets, the stronger a condition this imposes on $G$. One might say that the filter is fully generic if it meets \emph{every} dense set of $\mathbb P$, but the next result shows that this is too strong.
...
\begin{defn}
  Martin's axiom (MA) is the statement: For every ccc partial order $\mathbb P$ and every family $\mathcal F$ of fewer than continuum many dense subsets of $\mathbb P$ there exists an $\mathcal F$-generic filter $G\subset\mathbb P$.
\end{defn}

MA packages a great deal of forcing in one universe $V$. When MA is true, it is as if somebody has already performed forcing with all of the ccc partial orders. Of course since we can never find fully generic sets, there had to be some limitation on the genericity of $G$.
