...
Since ccc forcing preserves cardinals, if $V$ satisfies $\neg$CH then it is not possible to force CH using ccc forcing.
...
Like $\Fn(\kappa,\lambda)$, forcing with $\Fn_{\aleph_0}(\kappa,\lambda)$ adds a new function $g\colon\kappa\to\lambda$. But as we'll see, it does it in a different way. For instance it is easy to see that $\Fn_{\aleph_0}(\kappa,\lambda)$ is not ccc.
...
\begin{defn}
  A partial order $\PP$ is \emph{$\kappa$-closed} if
\end{defn}

\begin{lem}
  If $\PP$ is $\kappa$-closed, then $\PP$ preserves cardinals $\leq\kappa^+$.
\end{lem}
