...
\begin{proof}
  ...
  To establish the claim, let $S\in V[G]$ be a subset of $\omega$ and let $\dot S$ be a name for $S$. By the forcing theorem there is some $q\in G$ such that $q\forces\dot S\subset\check\omega$. Using Zorn's Lemma, we can find an antichain $A_n$ which is maximal with respect to the property that for all $p\in A_n$ we have $p\leq q$ and $p\forces\check n\in\dot S$. We will show that the corresponding nice name $\tau$ is a name for $S$.

  It is clear that $\tau_G\subset S$, since if $(\check n,p)\in G$ then $p\forces\check n\in\dot S$ and hence $V[G]$ satisfies $n\in S$. Conversely, if $V[G]$ satisfies $n\in S$ then there is some $q'\in G$ such that $q'\forces\check n\in\dot S$. As usual since $G$ is downwards directed we can suppose that $q'\leq q$. We now consider
  \[D=\set{p\in\PP\mid p\perp q'\text{ or }(\exists a\in A_n)\;p\leq a}
  \]
  Then $D$ is dense: given any $p_0$, if $p_0$ is incompatible with $q'$, then $p_0\in D$. Otherwise $p_0$ is compatible with $q'$ and hence there is $p_1\leq p_0,q'$. Since $p_1\forces\check n\in\dot S$ and $A_n$ is maximal with respect to this property, $p_1$ must be compatible with some element $a\in A_n$. Hence we can find $p_2\leq p_1,a$, and then $p_2\in D$.
