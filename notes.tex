...
In the previous sections we studied the effect of the forcing $\Fn(\kappa,\omega)$ on the value of the continuum. Next, we study the effect of the forcing $\Fn(\kappa,\omega)$ on the value of the other cardinal characteristics in Cicho\'n's diagram. Before we begin we need the following fact about $\Fn(\kappa,\omega)$, which says essentially that the forcing can be broken up into intermediate stages.

\begin{lem}
  Suppose that $\kappa$ is a cardinal, $I\subset\kappa$, and let $\PP=\Fn(\kappa,\omega)$, $\PP_1=\Fn(I,\omega)$ and $\PP_2=\Fn(\kappa\smallsetminus I,\omega)$. Let $G$ be a $\PP$-generic filter, and set $G_1=G\cap\PP_1$ and $G_2=G\cap\PP_2$. Then $G_1$ is $V$-generic for $\PP_1$, $G_2$ is $V[G_1]$-generic for $\PP_2$, and $V[G]=V[G_1][G_2]$.
\end{lem}

We leave the elementary proof of this result to the reader. [See Kunen, V.2.18]

Returning to the effect of forcing on Cicho\'n's diagram, our first result improves Theorem~\ref{thm:ch-exact} to show that not only $\mathfrak c$ but also $\mathfrak d$ is enlarged to $\kappa$.
...
\begin{proof}
  We already know that $\mathfrak d\leq\mathfrak c=\kappa$. Now suppose, towards a contradiction, that in $V[G]$ we have $\mathfrak d<\kappa$. Then in $V[G]$ there exists a dominating family $\mathcal F$ such that $|\mathcal F|<\kappa$.

Then, $\exists I\subseteq \kappa$ with $| I |<\kappa$ such that:
		$$\mathbb{P}_1=F_n(I, 2)$$
		$$\mathbb{P}_2=F_n(\kappa\setminus I, 2)$$
$\mathcal F$ appears in $V[G_1]$. 
But then, forcing with $\mathbb{P}_2$ adds a real \underline{not} dominated by $\{h_\alpha\}$, a contradiction.
\end{proof}
