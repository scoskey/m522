...
\[\Diff_P(x)=\set{y\in2^\omega\mid (\exists N)(\forall n\geq N)\;y\restriction I_n\neq x\restriction I_n}
\]
Proposition~\ref{prop:meager-diff} states that a subset $A\subset\omega^\omega$ is meager if and only if $A$ is contained in some set of the form $\Diff_P(x)$.

The key to the proof of the main theorem is the following lemma, which can be thought of as the analog of Lemma~\ref{lem:cohen-dominating}, where the $\leq^*$ relation is replaced by the ``different'' relation.
