...
We can use the following memory device to help us understand morphisms. The pair $(\phi,\psi)$ is a morphism if the following diagram ``commutes.'' The word ``comutes'' is used loosely, since the $R$ and $S$ edges symbolize relations, not functions.
\begin{center}
\begin{tikzpicture}
  \node (domr) {$\dom(R)$};
