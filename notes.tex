...
In this section we complete the proof of the two inequalities remaining in Theorem~\ref{thm:cichon}. These inequalities were discovered later than the others; they are a little more difficult too. In fact I will need your help proving some of the preparatory facts given in the exercises. The proof we give is due to Bartoszy\'nski, although the result was discovered also by Raisonnier--Stern.

We begin by introducing a new relation that sits between $\cof(\Null)$ and $\cof(\Meager)$. A \emph{slalom} is a sequence $S=(I_n)$ of subsets $I_n\subset\omega$ such that $|I_n|\leq2^n$. We write $SL$ for the set of slaloms. An element $f\in\omega^\omega$ is said to \emph{ski through} $S$, which we write $f\in^*S$, if for almost every $n$ we have $f(n)\in I_n$.
...
\begin{proof}[Proof of Lemma~\ref{lem:null-slalom}]
  ...
  To the second issue, we claim that the sum $\sum2^{-n}|I_n(s)|$ converges. Indeed, by the independence of the sets $E_{n,i}$ we have
...
\begin{exerc}
  ...
  Prove Proposition~\ref{prop:self-supporting}. [Hint: First show that there exists set $C$ which is disjoint from $A$, closed, and nonnull. Now let $D$ be the union of all $V_s$ such that $m(V_s\cap C)=0$, and let $K=C\smallsetminus D$. Then $K$ is closed and has the desired property.]
\end{exerc}

\begin{exerc}
  \label{exerc:pi-base-meager}
  Prove Proposition~\ref{prop:pi-base-meager}. [Hint: Wlog $O=2^\omega$ itself. Let $\mathcal U_1$ be any open basis. Proceed by induction. If $\mathcal U_n$ has been constructed, \ldots]
\end{exerc}
