...
In the previous sections we studied the effect of the forcing $\Fn(\kappa,\omega)$ on the value of the continuum. Next, we study the effect of the forcing $\Fn(\kappa,\omega)$ on the value of the other cardinal characteristics in Cicho\'n's diagram. In this section, we study only the effect of Cohen forcing on $\mathfrak b$ and $\mathfrak d$.

We begin by showing that $\Fn(\kappa,\omega)$ enlarges $\mathfrak d$ to $\kappa$. This improves the result of Theorem~\ref{thm:ch-exact}, which showed that $\Fn(\kappa,\omega)$ enlarges $\mathfrak c$ d to $\kappa$.

\begin{thm}
  \label{thm:cohen-b}
  Suppose that in $V$, CH is true and $\kappa$ is an uncountable cardinal such that $\kappa^{\aleph_0}=\kappa$. Let $\PP=\Fn(\kappa,\omega)$ and $G$ be a $V$-generic filter. Then in $V[G]$, we have $\mathfrak d=\kappa$.
\end{thm}

Before the proof we need the following fact about $\Fn(\kappa,\omega)$, which says essentially that the forcing can be broken up into two stages: an initial segment of the forcing followed by a tail segments of the forcing.
...
\begin{proof}
  It is straightforward to verify that $G_1$ and $G_2$ are filters. Moreover it is not difficult to verify that $G_1$ is $V$-generic for $\PP_1$. We will establish the slightly harder statement that $G_2$ is $V[G_1]$-generic for $\PP_2$.

  For this let $D\in V[G_1]$ be a dense subset of $\PP_2$, and let $\dot D$ be a $\PP_1$-name for $D$. Using the forcing lemma, we can find $p_1\in G_1$ such that $p_1\forces$ ``$\dot D$ is dense in $\check\PP_2$''. Let
  \[D'=\set{p\cup q\mid p\in\PP_1,\;q\in\PP_2,\text{ and either $p\perp p_1$ or else $p\leq p_1$ and $p\forces\check q\in\dot D$}}
  \]
  We claim that $D'$ is dense in $\PP$. Admitting this for the moment, it follows that there exists $r=p\cup q\in G\cap D'$. Then $q\in G_2$, and by definition of $D'$ we have that $q\in D$, which establishes the desired result that $G_2\cap D\neq\emptyset$.

  To show that $D'$ is dense, let $p\cup q$ be an arbitrary element of $\PP$, where $p\in\PP_1$ and $q\in\PP_2$. If $p\perp p_1$ then we are done, so suppose instead that $p\leq p_1$. Since $D$ is dense in $\PP_2$, there exists $q'\leq q$ such that $q'\in D$. Then by the forcing theorem there exists $p'\in G_1$ such that $p'\forces\check q'\in\dot D$. Letting $p''\leq p,p'$, we have that $p''\cup q'\in D'$, as desired.
\end{proof}

We are now ready to establish the theorem.

\begin{proof}[Proof of Theorem~\ref{thm:cohen-b}]
  We already know that $\mathfrak d\leq\mathfrak c=\kappa$. Now suppose, towards a contradiction, that in $V[G]$ we have $\mathfrak d<\kappa$. Then in $V[G]$ there exists  a dominating family $\mathcal F$ such that $|\mathcal F|<\kappa$. We claim that there is some $I\subset\kappa$ such that $|I|<\kappa$ and $\mathcal F$ is added by the forcing $\Fn(I,\omega)$.

  Assuming the claim is true, we can reach a contradiction as follows. As in the lemma, we let $PP_1=\Fn(I,\omega)$, $G_1=G\cap\PP_1$, $\PP_2=\Fn(\kappa\smallsetminus I,\omega)$, and $G_2=G\cap\PP_2$. Then the dominating family $\mathcal F$ lies in $V[G_1]$. But forcing with $\PP_2$ over $V[G_1]$ adds a generic element of Baire space $g_2=\bigcup G_2$, and an easy density argument shows that $g_2$ is not dominated by any real of $V[G_1]$. (See Exercise~\ref{exerc:cohen-nondom}.)
...
On the other hand, the forcing $\Fn(\kappa,\omega)$ does not raise the value of $\mathfrak b$.

\begin{thm}
  Suppose that in $V$, CH is true and $\kappa$ is a cardinal. Let $\PP=\Fn(\kappa,\omega)$ and $G$ be a $V$-generic filter. Then in $V[G]$, we have $\mathfrak b=\aleph_1$.
\end{thm}

\begin{proof}
  Since the Baire space of the ground model, $\omega^\omega\cap V$, is itself a family of size $\aleph_1$, it suffices to show that $\omega^\omega\cap V$ remains unbounded in $V[G]$. So suppose to the contrary that some $h\in V[G]$ is a bound for $\omega^\omega\cap V$.
  ...
  Next, using the forcing theorem, find an element $p\in\PP_1$ such that
  \[p\forces (\forall f\in\omega^\omega\cap V)\;f\leq^*\dot h\text{.}
  \]
  Since $\PP_1$ is countable, we can enumerate the elements $q\leq p$ in a list $\{q_i\}$. For each $i$ and $n$, we let
  \[m_{i,n}=\max\set{m\mid q_i\forces\check m\leq\dot h(\check n)}\text{.}
  \]
  Observe that since $q_i\forces h(n)\in\omega$, this maximum is well-defined. Therefore we can diagonalize against these sets to define the function $f(n)=\max\set{m_{0,n},\ldots,m_{n,n}}+1$.

  Now, since $f$ was defined entirely within $V$, we have that $p\forces \check f\leq^*\dot h$. In detail, this means that $p\forces(\exists N)(\forall n\geq N)\;\check f(n)\leq\dot h(n)$. By Lemma~\ref{lem:forcing-exists}, there exists $i$ and $N$ such that $q_i\forces (\forall n\geq\check N)\;\check f(n)\leq\dot h(n)$. 

  Finally, letting $n\geq N,i$ and $m=f(n)$, we have $q_i\forces\check m\leq\dot h(\check n)$. It follows that $m\leq m_{i,n}$, and by definition of $f$ we also have $m_{i,n}<f(n)$. This is a contradiction!
\end{proof}
...
\begin{thm}
  ...
  Suppose that in $V$, CH is true and $\kappa$ is an uncountable cardinal with $\kappa^{\aleph_0}=\kappa$. Then forcing with $\Fn(\kappa,\omega)$ results in all cardinals in the right half of Cicho\'n's diagram equal to $\kappa$, and all cardinals in the left half of Cicho\'n's diagram equal to $\aleph_1$.
\end{thm}
...
% differences between cohen and random forcing models
