...
We say that a set of axioms is \emph{consistent}, or consistent with ZFC, if there is a model of ZFC which satisfies those axioms. Thus the above result can be rephrased: MA+$\neg$CH is consistent.
...
\begin{proposition}
  In the definition of MA, we cannot completely drop the condition that the family of dense sets $\mathcal F$ has size less than continuum.
\end{proposition}
...
% Remark about switching from omega to 2
...
% Might want to elaborate on that a little more... why is the count \kappa^{\aleph_0}
...
% \end{lemma}
%
% [For the proof, and the definition of $p\hat{\times}q$, see Kunen V.4.30.]
