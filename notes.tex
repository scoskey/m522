...
Before we study the effect of the $\kappa$-sized Cohen forcing $\Fn(\kappa,\omega)$, we first examine the effect of the countable Cohen forcing $\Fn(\omega,\omega)$. The next result showcases the two-sided role that Cohen forcing plays with respect to the domination order $\leq^*$.
...
\begin{thm}
  ...
  Suppose that in $V$, CH is true and $\kappa$ is an uncountable cardinal with $\kappa^{\aleph_0}=\kappa$. Then forcing with $\Fn(\kappa,\omega)$ results in all cardinals in the left half of Cicho\'n's diagram equal to $\aleph_1$, and all cardinals in the right half of Cicho\'n's diagram equal to $\mathfrak c=\kappa$.
\end{thm}
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
  \caption{Values of the cardinal characteristics in Cicho\'n's diagram after Cohen forcing. Refer to Figure~\ref{fig:cichon} for the template of cardinal characteristics.}
\end{figure}

We have the following analogs of the statements in Lemma~\ref{lem:cohen-dominating}. For the statement we need our combinatorial characterization of meager sets by difference sets. Recall that if $P$ is a partition of $\omega$ into intervals and $x\in\omega^\omega$, then $\Diff_P(x)$ denotes the set of all $y$ which differ from $x$ on almost every interval of $P$. Recall also that a set $A$ is meager if and only if $A$ is contained in some set of the form $\Diff_P(x)$.

\begin{lem}
  \label{lem:cohen-meager}
  Let $\PP=\Fn(\omega,\omega)$, and let $G$ be a $V$-generic filter. Then we have:
  \begin{itemize}
  \item If $g=\bigcup G$, then $g$ does not lie in any set of the  form $\Diff_P(x)$ where $P,x\in V$. In other words, the generic $g$ does not lie in in any meager set coded in $V$.
  \item If $Q,y\in V[G]$, then not every element of $\omega^\omega\cap V$ lies in $\Diff_Q(y)$. In other words, the Baire space of the ground model does not become meager after the forcing.
  \end{itemize}
\end{lem}
