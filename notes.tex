...
\begin{exerc}
  With the definition of $C+C'$ for Dedekind cuts, show that addition is commutative and associative.
\end{exerc}
...
\item countably additivity: if $(A_n)_{n\in\N}$ is a sequence pairwise disjoint sets then $m(\bigcup A_n)=\sum m(A_n)$
\end{enumerate}

Perhaps surprisingly, the conditions (a)--(c) are mutually contradictory and so no such measurement function $m$ exists! Here is Vitali's simple example of a contradiction arising from these requirements. Regard $\Q$ as an additive subgroup of $\R$ and consider the cosets of $\Q$, that is, sets of the form $x+\Q$. Let $A\subset[0,1]$ be a set of coset representatives, that is, $A$ contains exactly one element from each of the cosets. (It is always possible to choose a coset representative in $[0,1]$ because $\Q$ is dense.)

Then the translations $q+A$ for $q\in\Q$ form a countable sequence of pairwise disjoint sets that cover all of $\R$. In fact, if we let $S=\Q\cap[-1,1]$ then the translations $q+A$ for $q\in S$ already cover all of $[0,1]$. We can then infer from (a) and (c) that $m[\bigcup_{q\in S}(q+A)]$ lies between $1$ and $4$. But on the other hand, by (b) and (c) we have that 
...
\begin{exerc}
  Show that there is a bijection between $\mathcal P(\N)$ and $\R$.
\end{exerc}
...
\begin{remark}
  Among the spaces we've explored so far, the notion of $\sigma$-compactness only works in the Baire space. That's because both $2^\omega$ and $\R$ both have the property that the whole space is $\sigma$-compact: $\R$ is the union of closed bounded intervals, and $2^\omega$ is itself compact.
\end{remark}
...
\begin{remark}
  ...
  Similarly, the $\Null$ symbol can stand for the null ideal on either $\R$ or $2^\omega$; see Exercise~\ref{exerc:null-bij}. In fact, it is possible to define a natural measure on $\omega^\omega$, and then $\Null$ can stand for the null ideal on $\omega^\omega$ too. [ref] % blass
\end{remark}
...
% As was the case with Lemma~\ref{lem:random-conull}, the following result can be stated for Borel sets.
...
% \begin{proof}
%   First suppose that $A$ is open. Clearly such a set cannot be null, but we will show that $A$ and $A^*$ have the same measure. First, express $A$ as a union $\bigcup V_{s_n}$. By throwing away any summands that are contained in another, we can suppose without loss of generality that the $V_{s_n}$ are disjoint. Thus the measure of both $A$ and $A^*$ are equal to $\sum2^{-|s_n|}$, as desired.

%   Now suppose that $A=\bigcap_m\bigcup A_m$ where $A_m$ are open, and suppose that $A$ is null. By continuity from above (see Exercise~\ref{exerc:continuity-above}), we have $m(A)=\inf m(A_m)=0$. By the same reasoning in $V[G]$, we have $m(A^*)=\inf m(A_m^*)$, and by the result of the previous paragraph, the latter quantity equals $\inf m(A_m)=0$, as desired.
% \end{proof}
