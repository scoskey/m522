...
\begin{defn}
  ...
  \begin{itemize}
  \item $\alpha$ is well-ordered by the relation $\beta\leq\gamma$ iff $\beta\in\gamma$ or $\beta=\gamma$;
  \item $\alpha$ is transitive: if $\beta\in\gamma\in\alpha$ then $\beta\in\alpha$.
  \end{itemize}
\end{defn}
...
An ordinal $\alpha$ is said to be \emph{successor} if it is of the form $\beta+1$ for some ordinal $\beta$. Otherwise $\alpha$ is said to be \emph{limit} because of the following fact.

\begin{prop}
  Every limit ordinal is the union of the ordinals that came before it.
\end{prop}

\begin{proof}
  Let $\alpha$ be a limit ordinal, and let $\alpha'$ be the union of all $\beta\in\alpha$. Clearly since $\alpha$ is transitive we have $\alpha'\subset\alpha$. On the other hand if $\beta\in\alpha$ then $\beta+1\leq\alpha$ and since $\alpha$ is limit we must have $\beta+1<\alpha$. Thus $\beta\in\beta+1\in\alpha$ and it follows that $\beta\in\alpha'$.
\end{proof}
...
The two most important examples of sequences spaces are the \emph{Cantor space} $2^\omega$ and the \emph{Baire space} $\omega^\omega$.
...
For the Baire space we use some facts from the theory of continued fractions.
...
To begin, let $A^{<\omega}=\bigcup A^n$ denote the set of finite sequences of elements of $A$. This set is partially ordered by extension of sequences. A subset of $A^{<\omega}$ is said to be a \emph{tree} if it is closed downward in this order, that is, closed under initial segments. If $T\subset A^{<\omega}$ is a tree then an element $x\in A^\omega$ is said to be a \emph{branch} through $T$ if all finite initial segments of $x$ lie in $T$.
...
\begin{proof}
  First suppose $C$ is closed and let $T$ be the tree of all finite initial segments of elements of $C$. Then it is clear that every element of $C$ is a branch through $T$. On the other hand if $x$ is a branch through $T$ then for all $n$ we can find $x_n\in C$ such that $x,x_n$ share a common initial segment of length $n$. It follows that $x_n\to x$, and since $C$ is closed we have $x\in C$.

  Conversely, suppose that $T\subset A^{<\omega}$ is a tree and $C$ is the set of braches through $T$. If $x\notin C$, then there must be some finite initial segment $s\subset x$ such that $s\notin T$. It follows that the basic open set $V_s$ is disjoint from $C$. This shows that the complement of $C$ is open, and hence that $C$ is closed.
\end{proof}
...
In particular the notions of meager and comeager sets make perfect sense in both $2^\omega$ and $\omega^\omega$.
