...
The proposition implies that the collection of sets $\Diff_P(x)$ forms a basis for the meager ideal on Cantor space. This basis is particularly useful because it is easy to see when two sets of the form $\Diff_P(x)$ are related by the subset relation. 
...
The proof is elementary (assuming I have not made a typo) and we leave it for Exercise~\ref{exerc:engulf}. Proposition~\ref{prop:engulf} provides characterization of $\cof(\Meager)$ as the dominating number of a simple combinatorial relation on chopped reals. The cardinals $\mathfrak b$ and $\mathfrak d$ can be characterized by a similar combinatorial relation just on interval partitions. First, if $P=(I_n)$ and $Q=(J_m)$ are interval partitions, we write $P\prec Q$ if for all but finitely many $J_m$ there exists $I_m$ such that $I_n\subset J_m$. 

\begin{prop}
  We have $\mathfrak d$ is the dominating number of the $\prec$ relation on interval partitions and $\mathfrak b$ is the dominating number of the $\not\succ$ relation on interval partitions. In fact, there is a morphism from $\leq^*$ to $\prec$ and vice versa.
\end{prop}
...
Although this last result was not a morphism-type proof, it is true that an analogous argument shows the dual property that $\add(\Meager)=\max\{\mathfrak d,\non(\Meager)\}$. This three-term duality is once again a part of an abstract morphism framework, but it is a little more involved than the morphisms we introduced in the previous section. [ref]
