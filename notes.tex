...
\begin{defn}
  Let $\PP$ be a partial order in $V$ and $G\subset\PP$ be a filter. If $\tau$ is a $\mathbb P$-name, then we define $\tau_G$ recursively by
...
\begin{proof}
  It is clear that $V[G]$ is countable is clear from its definition; each element of $V[G]$ is determined by a name which lies in $V$.

  For the statement about ordinals, first recall that the property of being an ordinal is absolute to transitive models. Now suppose that $\alpha$ is an ordinal of $V[G]$ and let $\tau$ be a name such that $\alpha=\tau_G$. Recall that for any set $x$ we can define the $\in$-rank $\rk(x)=\sup\set{\rk(y)\mid y\in x}$. Then it is easy to see inductively that $\rk(\tau_G)\leq\rk(\tau)$, and hence $\alpha\leq\rk(\tau)$. Since $V$ is transitive it follows that $\alpha\in V$.
\end{proof}
...
\begin{lem}
  If $\mathbb P$ is a partial order in $V$ and $G\subset\mathbb P$ is a filter, then the extension $V[G]$ satisfies the axioms of foundation, extensionality, pairing, and union.
\end{lem}

\begin{proof}
  Every set satisfies the axiom of foundation. To show $V[G]$ satisfies extensionality, it is enough to show that $V[G]$ is a transitive set. For this, let $\tau_G$ is an arbitrary element of $V[G]$ and $x\in\tau_G$. Then by definition of $\tau_G$, we have $x=\sigma_G$ for some $\sigma\in\dom(\tau)$. In particular, $x\in V[G]$.

  For pairing and union, we simply must devise names for these constructions. First suppose $\sigma_G,\tau_G\in V[G]$ and let $\rho$ denote the name $\set{(\sigma,\mathbb 1),(\tau,\mathbb 1)}$. Then it is immediate to check that $\rho_G=\{\sigma_G,\tau_G\}$. Next suppose $\tau_G\in V[G]$, and let $\sigma=\set{}$. Then it is again straightforward to check that $\bigcup\tau_G\subset\sigma_G$. (To hit $\bigcup\tau_G$ exactly we can either refine this argument or appeal to the comprehension axiom after we have proved it.)
\end{proof}
...
Let us first see the power of forcing theorems (a) and (b) to establish forcing theorem (b).

\begin{thm}
  Assuming forcing theorems (a) and (b), we show that $V[G]$ satisfies the comprehension axiom.
\end{thm}
...
The remaining axioms of ZFC will be left to the reader :)
