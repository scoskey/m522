...
Before the proof we need the following fact about $\Fn(\kappa,\omega)$, which says essentially that the forcing can be broken up into two steps: an initial segment of the forcing followed by a tail segments of the forcing.

\begin{lem}
  Let $\PP=\Fn(\kappa,\omega)$ and let $G$ be a $V$-generic filter for $\PP$. Suppose that $\alpha$ is an ordinal less than $\kappa$, and set $\PP_1=\Fn(\alpha,\omega)$ and $G_1=G\cap\PP_1$; likewise set $\PP_2=\Fn(\kappa\smallsetminus\alpha,\omega)$ and $G_2=G\cap\PP_2$. Then $G_1$ is $V$-generic for $\PP_1$, $G_2$ is $V[G_1]$-generic for $\PP_2$, and $V[G]=V[G_1][G_2]$.
\end{lem}
...
\begin{proof}[Proof of Theorem~\ref{thm:cohen-b}]
  ...
  \]
  We then let $\tau$ be a nice name for $S_{\mathcal F}$, and note that since $\PP$ is ccc there are only $\lambda$ many elements of $\PP$ mentioned in $\tau$. Since $\lambda<\kappa$
\end{proof}
...
\begin{exerc}
  ...
  Show that if $\PP=\Fn(\omega,\omega)$ and $G$ is $V$-generic, then $g=\bigcup G$ is not dominated by any element of $V$.
\end{exerc}
