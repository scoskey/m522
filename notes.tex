...
\begin{proof}[Proof of Lemma~\ref{lem:null-slalom}]
  ...
  \[\phi(f)=\bigcup_N\bigcap_{n\geq N} E_{n,f(n)}\;.
  \]
  Then it is clear that $\phi(f)$ is a null set.

  On the other hand given a null set $A\subset2^\omega$ we will define a slalom $\psi(A)$ in several steps. We first find a closed subset $K\subset2^\omega\smallsetminus A$ with the properties given in Proposition~\ref{prop:self-supporting}. We then let
  \[I_n(s)=\set{i\in\omega\mid K\cap V_s\neq\emptyset\text{ and }K\cap V_s\cap E_{n,i}=\emptyset}
  \]
  We claim that the sum $\sum|I_n(s)|/2^n<\infty$. Indeed, notice that $\prod>0$. Hence we can take a logarithm to obtain. Looking at the Taylor series we always have $-\log(1-x)>x$, so it follows that $\sum|I_n(s)|/2^n<\infty$, as claimed.

  The last step in the construction is to find a single slalom $\psi(A)=(I_n)$ that dominates all of the sequences $(I_n(s))$. For this fix an enumeration $s_1,s_2\ldots$ of the elements of $\omega^{<\omega}$. By the convergence shown in the previous paragraph we can find $k(s_j)$ such that for all $n\geq k(s_j)$ we have $|I_n(s)|\leq2^n/2^j$. We then let $I_n=\bigcup\set{I_n(s_j)\mid k(s_j)\leq n}$. Then clearly
  \[|I_n|\leq\sum_{k(s_j)\leq n}|I_n(s_j)|\leq\sum2^n/2^j\leq2^n
  \]
  Thus $\psi(A)=(I_n)$ is a slalom, and this completes the construction of $\psi$.

  To conclude we need to argue that for all $f\in\omega^\omega$ and $A\in\Null$ we have $\phi(f)\subset A$ implies $f\in^*\psi(S)$.
\end{proof}
