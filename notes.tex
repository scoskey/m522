...
By applying this result to the family $\mathcal F=\{A_0,A_1\}$, we conclude that either $|A_0|\leq|A_1|$ or $|A_1|\leq|A_2|$, and hence the total ordering property holds. We also conclude the well-ordering property, except for the small technicality that the least element property should also apply when $\mathcal F$ is a proper class. In the proof we will need the following consequence of the axiom of choice.
...
%We conclude by briefly giving a more rigorous set-theoretic definition of the $\aleph$ numbers. First, it is necessary to describe the ordinals, which appear as the subscripts $\aleph_\alpha$. While cardinals like $\aleph_3$ measure the \emph{size} of a set, ordinals like $\alpha$ measure the \emph{position} in an enumeration.

%The first few ordinals are defined by $0=\emptyset$, $1=\{\emptyset\}$, and $n+1=n\cup\{n\}$. After infinitely many ordinals have been constructed, the next ordinal is defined as the union of all those that came before it.
