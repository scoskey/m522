...
\begin{example}
  The partial order $\mathbb P=$ is ccc, because\ldots
\end{example}

\begin{example}
  The partial order $\mathbb P=$ is not ccc, because\ldots
\end{example}
...
\begin{proof}
  This time we let use the partial order $\omega_1^{<\omega}$ with the upside-down order. It is clear that this partial order is not ccc, since even the family of elements $s\in\omega_1^{<\omega}$ such that $|s|=1$ is an uncountable antichain. Now we consider the sets
...
The \emph{amoeba} partial ordering is\ldots

\begin{lem}
  The amoeba partial ordering is ccc.
\end{lem}
...
\begin{proof}
  It suffices to show that any uncountable $\mathcal F\subset\Fn(\kappa,\omega)$ contains two compatible elements. In fact we will show that it contains uncountably many pairwise compatible elements. For this we first consider the corresponding family of domains $\mathcal G=\set{\dom(f)\mid f\in\mathcal F}$. Then $\mathcal G$ is an uncountable family of finite subsets of $\kappa$. Since there are only countably many possible values of $|F|$ for $F\in\mathcal G$, by the pigeon-hole principle we can replace $\mathcal G$ with an uncountable subset of it such that $|F|=$ some fixed value $n$ for all $F\in\mathcal G$.

  We now claim that there exists an uncountable subset $\mathcal G_0\subset G$ and a finite set $A\subset\kappa$ such that for every $F,F'\in\mathcal G_0$ we have $F\cap F'=A$. (For future notation, a family $\mathcal G_0$ with this property is called a \emph{delta system} with root $A$.) For this we use induction on $n$. If $n=1$, then $\mathcal G$ forms a delta system automatically.

  Next suppose that the claim is true for families of sets of size $<n$. First consider the case that there is some $\alpha\in\kappa$ which lies in uncountably many of the $F\in\mathcal G$. Then by the inductive hypothesis we can find a delta system $\mathcal H_0$ contained in $\set{F\smallsetminus\{\alpha\}\mid F\in\mathcal G}$. It follows that the family $\mathcal G_0=\set{F_0\cup\{\alpha\}\mid F_0\in\mathcal H_0}$ is a delta system too.
  ...
  Finally, let $\mathcal F_0\subset\mathcal F$ be the subset of $\mathcal F$ consisting of functions $f$ such that $\dom(f)\in\mathcal G_0$. These functions may disagree on $A$, but never disagree elsewhere. However since there are only countably many possibilities for $f\restriction A$, by the pigeon-hole principle we can replace $\mathcal F_0$ with an uncountable subset of it such that $f\restriction A=$ some fixed function $f_0$ for all $f$. It follows from this that every pair of elements of $\mathcal F_0$ is compatible.
\end{proof}

The trick with the delta system arises frequently in the verification of the ccc property; it is called the \emph{delta system lemma}.
