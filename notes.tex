...
[ref] % i guess the same as before about SN, but also blass with simpler examples
...
\begin{proof}
  Beginning with the implication $(\Leftarrow)$, we will show $\Diff_P(x)$ is meager. For this, let
  \[A_N=\set{y\in2^\omega\mid(\forall n\geq N)\;y\restriction I_n\neq x\restriction I_n}\text{.}
  \]
  The set $A_N$ is a countable intersection of boolean combinations of basic open sets, and hence closed. Moreover, $A_N$ contains no nonempty basic open sets, and so $A_N$ is nowhere dense. Finally $\Diff_P(x)\subset\bigcup A_N$, and thus $\Diff_P(x)$ is meager.

  For $(\Rightarrow)$, suppose $A=\bigcup A_n$ where each $A_n$ is nowhere dense, and suppose without loss of generality that $A_n\subset A_{n+1}$ for all $n$. We will define an interval partition $P=(I_n)$ and an element $x\in2^\omega$ recursively. To begin, since $A_0$ isn't dense there exists $s\in2^{<\omega}$ such that $A_0\cap V_s=\emptyset$. We let $I_0=\dom(s)$ and $x\restriction I_0=s$.
