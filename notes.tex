...
\emph{cardinality}, which asks for instance whether a set is finite, countably infinite, or uncountable.
...
\item To a topologist, a key notion of size is \emph{category}, which asks whether a set is meager, comeager, or nonmeager non-comeager.
\end{itemize}

In this course we will ask: how do these three kinds of size compare with one another? In answering this question, we will discover numerous relationships between measure and category, with the relationships giving rise to a beautiful structure.
...
\begin{defn}
  A set of real numbers $A$ is called \emph{meager} if it is a union of countably many nowhere dense sets.
\end{defn}
...
\begin{thm}[Baire category theorem]
  The set of all real numbers $\RR$ is not meager.
\end{thm}
