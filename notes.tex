...
In fact we'll see shortly that $\non(\mathcal K_\sigma)\leq\cov(\mathcal K_\sigma)$. Meanwhile the theorem implies the analog of the Baire category theorem for $\sigma$-compact sets: if $V_s$ is any basic open set then $V_s$ is not $\sigma$-compact. Indeed, observe that $V_s$ is homeomorphic to $\omega^\omega$ itself, and $\omega^\omega$ is not $\sigma$-compact because that would imply that $\cov(\mathcal K_\sigma)=1$.
...
\begin{proof}
  For the first equality, we show that if $\mathcal F\subset\omega^\omega$ then $\mathcal F$ is $\leq^*$-bounded if and only if $\mathcal F$ is a subset of a $\sigma$-compact set. For this we will use the following notation: for $g\in\omega^\omega$ we let $A_g=\prod_n\{0,\ldots,g(n)\}$. Observe that $f\leq g$ if and only if $f\in A_g$.

Now if $\mathcal F$ is bounded by $h\in\omega^\omega$ then let $h_i$ enumerate all finite modifications of $h$. Then for all $f$ there is some $i$ such that $f\leq h_i$, and hence $\mathcal F\subset\bigcup_iA_{h_i}$. Conversely, suppose that $\mathcal F\subset\bigcup A_i$, where each $A_i$ is compact. Then for each $i$ there exists some $h_i\in\omega^\omega$ such that $A_i\subset\prod_n\{0,\ldots,h_i(n)\}$, in other words $A_i\subset A_{h_i}$. Since $\aleph_1\leq\mathfrak b$, there exists $h$ such that $h_i\leq^*h$ for all $i$. It follows that $\mathcal F$ is $\leq^*$-bounded by $h$.  
\end{proof}
