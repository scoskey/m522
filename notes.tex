...
\begin{cor}[Bartoszy\'nski]
  We have $\add(\Null)\leq\add(\Meager)$ and $\cof(\Meager)\leq\cof(\Null)$.
\end{cor}

This concludes the proof of Theorem~\ref{thm:cichon}.
...
Forcing is a method of adding new sets to a given model $V$ of ZFC, to obtain a larger model $W$ of ZFC. By carefully choosing \emph{how} we add the new sets, we may be able to \emph{force} a given statement to hold in $W$.

For example, \ldots

Note that forcing cannot do everything. For example, if $A\in V$ and $A\neq\emptyset$, then $A$ will remain nonempty after forcing. There is some $a$ such that $a\in A$ and this element will remain in $A$ forever. Moreover, if $A$ is a finite or even countable set, then it is not possible to add new elements to $A$ by any means.
...
We think of the dense subsets of $\mathbb P$ as omnipresent---almost every element of $\mathbb P$ lies below some element of $D$. When we force with $\mathbb P$ we add a filter $G$ that is \emph{generic} in the sense that it meets these dense subsets of $\mathbb P$. The more dense sets that we demand that $G$ meets, the stronger a condition this imposes on $G$. The filter is fully generic if it meets \emph{every} dense set of $\mathbb P$.

\begin{prop}
Suppose that for every $p\in\mathbb P$ there exist incompatible  then $\mathbb P$ does not have a generic filter.
\end{prop}
...
Such a partial order $\mathbb P$ is called \emph{atomless}; this nontriviality condition is necessary assumption for forcing with $\mathbb P$. Once we acknowledge that fully generic filters do not exist in the given universe of set theory, we arrive at the following definition.
...
\begin{example}
  Returning to the example of the forcing $\mathbb P=\omega^\omega$ with the upside-down ordering, we have already said that a filter $G\subset\omega^\omega$ is actually a path through $\omega^\omega$.
\end{example}
...
\begin{defn}
  A notion of forcing $\mathbb P$ is said to be \emph{ccc} (or satisfy the \emph{countable chain condition}) if every antichain $A\subset\mathbb P$ is countable.
\end{defn}
