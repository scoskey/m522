...
\begin{proof}
  Begin by letting $(F_{n,i})_{n,i\in\omega}$ be a partition of $\omega$ into subsets with $|F_{i,n}|=n$. Then the events $E_{n,i}=\set{x\in2^\omega\mid x\restriction I_{n,i}=0}$ are jointly independent and $m(E_{n,i})=2^{-n}$ for all $n,i$.

  Given $f\in\omega^\omega$ we let $\phi(f)$ be the null set $\liminf E_{n,f(n)}$.

  Given a null set $A$ we define the slalom $\psi(A)$ as follows.
\end{proof}

\begin{rem}
  Although we shall not need it, there is also a morphism from $\in^*$ to the $\subset$ relation on $\Null$. Thus the ski-through relation on slaloms can be used to give complete combinatorial characterizations of both $\add(\Null)$ and $\cof(\Null)$.
\end{rem}
