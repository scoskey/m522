...
\begin{proof}[Proof of Lemma~\ref{lem:null-slalom}]
  ...
  To define $\phi$, we start with a sequence of subsets $E_{n,i}\subset2^\omega$ such that $m(E_{n,i})=2^{-n}$, and given $f\in\omega^\omega$ we let
...
A \emph{notion of forcing} is a partial order $\mathbb P$ with a maximum element $1_{\mathbb P}$.
...
\begin{example}
  Let $T\subset\omega^{<\omega}$ be a tree. Then $T$ can be made into a notion of forcing by turning it upside-down, that is, letting $s\leq t$ if and only if $s\supset t$.
\end{example}


\begin{defn}
  A subset $D\subset\mathbb P$ is \emph{dense} if for all
\end{defn}
