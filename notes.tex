...
\begin{notes}
  The material of this section is borrowed from Blass, Section~4.
\end{notes}
...
Forcing as a black box\ldots

A subset $A\subset\mathbb P$ is said to be an \emph{antichain} if for all $p,q\in A$ we have $p$ and $q$ are not compatible. Thus if $A$ is an antichain, a filter $G$ can only choose at most \emph{one} of the elements of $A$.
...
Hechler forcing is always ccc, because incompatible conditions cannot have the same stem $s$, an there are only countably many possible stems $s$.
...
\begin{proof}
  Suppose towards a contradiction that $\mathcal F$ is an unbounded family of size $<\mathfrak c$, and let $\mathbb P$ be the Hechler forcing $\mathbb H_{\mathcal F}$. For each $f\in\mathcal F$ we consider the set
  \[D_f=\set{(s,F)\in\mathbb H_{\mathcal F}\mid f\in F}
  \]
  Then $D_f$ is dense, since any pair $(s,F)$ can be extended to the stronger element $(s,F\cup\{f\})$. Hence by MA we cand find a filter $G$ that meets $D_f$ for all $f\in\mathcal F$, and let
\[g=\bigcup\set{s\mid(\exists F)\;(s,F)\in G}
\]
be the element of Baire space determined by $G$. The partial order on $\mathbb H_{\mathcal F}$ then ensures that $g$ dominates $f$ for all $f\in\mathcal F$, contradicting that $\mathcal F$ is unbounded.
\end{proof}
% don't forget D_n
...
For this proof, we need a new notion of forcing. The \emph{amoeba} partial ordering $\mathbb A_\epsilon$ with parameter $\epsilon>0$ consists of all open subsets $O\subset\RR$ such that $m(O)<\epsilon$. The ordering on $\mathbb A_\epsilon$ is defined by $O\leq O'$ iff $O\supset O'$. Thus the elements of $\mathbb A_\epsilon$ are inner approximations to an open set of measure $\leq\epsilon$.
...
\begin{proof}
  Assume otherwise, then there is a $p_\psi\in \mathbb{P}$, $\psi<\omega_1$ such that:
		$$\psi\not=\nu \implies p_\psi\perp p_\nu.$$
Since $p_\psi\in \mathbb{P}$ we have that $p_\psi$ is open and $|p_\psi|<\epsilon$ and also that $\psi\not=\nu$ implies that $|p_\psi\cup p_\nu|\geq \epsilon$, by definition $\mathbb{P}$. 

Now, fix $\delta>0$ such that the set
		$$E:=\{ \psi<\omega_1: |p_\psi|\leq \epsilon-2\delta\}$$
is uncountable. 

Let $\mathcal{B}$ be the set of all finite unions of rational intervals, (Note that $\mathcal{B}$ is countable). Now, for each $\psi\in E$, choose $W_\psi\subseteq p_\psi$ with $W\psi\in \mathcal{B}$ and $|p_\psi\setminus W_\psi|\leq \delta$. Then, for $\psi, \nu\in E$ with $\psi\not=\nu$, we have $|W_\psi\cup W\nu|\geq \epsilon-2\delta$ and $|W_\psi|\leq |p_\psi|\leq \epsilon - 2\delta$. But $\mathcal{B}$ is countable, so we fix $\psi, \nu\in E$ with $\psi\not=\nu$ but $W\psi=W_\nu$, a contradiction.
\end{proof}

We are now prepared to prove Theorem~\ref{thm:ma-addn}.

\begin{proof}[Proof of Theorem~\ref{thm:ma-addn}]
  Fix $\kappa<\mathfrak{m}$. We want to show that $\kappa<\add(\mathcal{L})$. Let $N_\alpha$, for $\alpha<\kappa$ denote Lebgsgue null sets. Note that, in this proof, we use $|\cdot|$ to denote the typical Lebesgue measure. Ultimately, we want to show that $N_\alpha$ is null. Thus, given $\epsilon>0$, we want to find a set $U$ such that $|U|<\epsilon$ and $U\supseteq \bigcup_{\alpha<\kappa} N_\alpha$. To this end, fix $\epsilon>0$. 

  Next, for any filter $G$, let $U_G=\bigcup G$. $U_G$ is open, and we want to show that $|U_G|\leq \epsilon$ in order to prove $\kappa<\add(\mathcal{L})$. Since $U_G\subseteq \RR$, we have that $U_G$ is a second countable space, thus it is Lindel\"of. (i.e. there is a countable subset $G$, $\{p_j: j\in \omega\}$, whose union covers $U_G$.). By Lemma 5.15, $G$ is centered, so for each $n$ there is a $q\in\mathbb{P}$ such that $q\leq p_j$ for all $j<n$. Since $\leq$ is $\supseteq$, this implies that the measure of each $|\bigcup_{j<n}p_j|<\epsilon$. Since $U_G=\bigcup_{j\in\omega} p_j$ and the fact that measure is additive, we have $|\bigcup_{j\in\omega}|=|U_G|\leq \epsilon$. 

  Next, to apply $MA_\mathbb{P}(\kappa)$, we need a dense set: $D_\alpha=\{q\in\mathbb{P}: q\supseteq N_\alpha\}$. If $G\cap D_\alpha\not= \emptyset$ for all $\alpha<\kappa$ then $U_G\supseteq \bigcup_{\alpha<\kappa} N_\alpha$ and so, applying Martin's Axiom we are done. Thus, it remains to prove that $D_\alpha$ is dense. Fix $\alpha$ and $p\in\mathbb{P}$. Let $V$ be an open set containing $N_\alpha$ and $|V|<\epsilon-|p|$, and let $q=p\cup V$. Then we have $|q|<\epsilon$ and $q\in\mathbb{P}$. We also have that $q$ extends $p$, $q\leq p$, and $q\in D_\alpha$.
\end{proof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\section{Iterated Cohen and random forcing}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
