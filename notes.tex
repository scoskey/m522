...
The axiom MA has a lot to say about the combinatorics of the continuum, and specifically about the values of cardinal characteristics. We close this section with a sample application.
...
Using the machinery of interval partitions and the strong difference relation, the above result can be improved slightly.

\begin{thm}
  Assuming MA, we have $\cov(\Meager)=\mathfrak c$.
\end{thm}
...
\begin{proof}
  ...
  Now suppose towards a contradiction that $\cov(\Meager)<\mathfrak c$, and fix a family $\mathcal F$ of pairs $(y,P)$ as above. We will apply MA to the partial order $\mathbb P=2^{<\omega}$ to obtain a suitably generic filter $G$. Our goal will be to ensure that $G$ matches (doesn't strongly differ from) all of the $(y,P)$ pairs, which will be a contradiction. To this end, consider the subsets
  \[D_{y,P,n}=\set{s\in2^{<\omega}\mid \text{there is $I\in P$ such that $n<I$ and $s\restriction I=y\restriction I$}}
  \]
  Then for any $y$ and $P$, the set $D_{y,P}$ is dense: given any $s$ we can find an interval $I$ of $P$ beyond both $n$ and $\dom(s)$, and extend $s$ to a sequence $t$ which agrees with $y$ on $I$. 
...
In the next section we will see that with a more clever partial ordering, we can in fact show MA pushes all the cardinal characteristics in Cicho\'n's diagram up to $\mathfrak c$.
...
Next we explore random forcing.
