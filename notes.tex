...
The sequence space $A^\omega$ has a natural metric.
\[d(x,y)=\sum_{i\in\omega}\frac{1}{2^{i+1}}\delta(x(i),y(i))
\]

\begin{rem}
  While we have said that this metric is ``natural'', it is not really canonical because it gives different weights to different points in the sequence. In other words, reordering $\omega$ would give rise to a different compatible metric.
\end{rem}
