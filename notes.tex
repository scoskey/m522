...
\begin{rem}
  We remark that the sense in which $\forces$ is definable is somewhat technical. Technically it means that for each formula $\phi(x_1,\ldots,x_n)$ in the language of set theory, the relation on tuples $(p,\tau_1,\ldots,\tau_n)$ defined by $p\forces\phi(\tau_1,\ldots,\tau_n)$ is definable.
\end{rem}
...
We conclude this section with some elementary but frequently used properties of the forcing relation. These properties will also be used to guide the proof of the forcing theorems in the next section.
...
It takes a moment to confirm for oneself that the above definition does indeed determine $p\forces_0\phi$ for arbitrary $\phi$.

The proof of the forcing theorems will unfold over the next several lemmas. The first of these states that the analog of Lemma~\ref{lem:forcing-downward} holds for $\forces_0$.
...
\begin{proof}
  We prove this by induction on the complexity of the formula $\phi$. For example, let us check that the induction passes through the first clause of Definition~\ref{defn:forces-0}. Suppose $\sigma,\tau$ are names and that the lemma holds for the sentence $\sigma'=\sigma$ whenever $\sigma'\in\dom(\tau)$. Then if $p\forces_0\sigma\in\tau$, there is $(\sigma',q)\in\tau$ such that $p\leq q$ and $p\forces_0\sigma'=\sigma$. If also $p'\leq p$, then clearly also $p'\leq q$ and by inductive hypothesis $p'\forces_0\sigma'=\sigma$. Hence $p'\forces_0\sigma\in\tau$, as desired. The remaining four clauses are left for Exercise~\ref{exerc:forcing-downward-clauses-2345}.
\end{proof}
...
\begin{proof}
  First note that if the lemma holds for $\phi$ and $q\in G$ is given, then we can in fact find $p\leq q$ such that $p$ satisfies the conclusion of the lemma. To see this, simply note that if $p'\in G$ and $p'\forces_0\phi$, then we can find $p\in G$ with $p\leq p',q$ and by Lemma~\ref{lem:forcing-downward-0} we have also $p\forces_0\phi$.
% streamline and merge this cumbersome note into the next paragraph?

  Now the lemma itself is proved by induction on the complexity of $\phi$. To check that the induction passes through the first clause of Definition~\ref{defn:forces-0}, suppose that $\sigma,\tau$ are names and that the lemma holds for the sentence $\sigma'=\sigma$ whenever $\sigma'\in\dom(\tau)$. Then $V[G]$ satisfies $\sigma\in\tau$ if and only if there exists $(\sigma',q)\in\tau$ such that $V[G]$ satisfies $\sigma'=\sigma$. By inductive hypothesis together with the previous paragraph, this latter property holds if and only if there exists $(\sigma',q)\in\tau$ and $p\in G$ such that $p\leq q$ and $p\forces_0\sigma'=\sigma$. In other words, if and only if there is $p\in G$ such that $p\forces_0\sigma\in\tau$.

  The verification that the induction goes through the second, fourth, and fifth clauses of Definition~\ref{defn:forces-0} is straightforward and left for Exercise~\ref{exerc:forcing-clauses-245}.

  Finally, we verify that the induction goes through the third clause of Definition~\ref{defn:forces-0}. So supposing that the lemma is true for $\phi$, we will show that $V[G]$ satisfies $\neg\phi$ if and only if there is $p\in G$ such that $p\forces_0\neg\phi$. For this first suppose that there is $p\in G$ such that $p\forces_0\neg\phi$. Then for all $q\leq p$ we have $q\not\forces_0\phi$. It follows from Lemma~\ref{lem:forcing-downward-0} that for all $q\in G$ we have $q\not\forces_0\phi$. By inductive hypothesis, we must have that $V[G]$ does not satisfy $\phi$, and hence $V[G]$ satisfies $\neg\phi$.

  Conversely suppose that $V[G]$ satisfies $\neg\phi$. Then $V[G]$ does not satisfy $\phi$, so by inductive hypothesis, for all $p\in G$ we have $p\not\forces_0\phi$. We now let $D=\set{p\in\PP\mid p\forces_0\phi\text{ or }p\forces_0\neg\phi}$; it is immediate from the definition of $p\forces_0\neg\phi$ that $D$ is dense. It follows that there exists $p\in G\cap D$, and since $p\not\forces_0\phi$ we must have $p\forces_0\neg\phi$, as desired.
\end{proof}
...
Just as the forcing $\omega^{<\omega}$ uses finite approximations to add a new function $\omega\to\omega$, the forcing $\Fn(\kappa,\lambda)$ uses finite approximations to add a new function $\kappa\to\lambda$. Our first result shows that since the approximations are finite, $\Fn(\kappa,\lambda)$ also adds lots of new functions from $\omega\to\lambda$.

\begin{lem}
  Forcing with the partial order $\PP=\Fn(\kappa,\omega)$ adds $\kappa$ many new elements to $\omega^\omega$.
\end{lem}

\begin{proof}
  Let $G\subset\PP$ be a $V$-generic filter, and let $g=\bigcup G$. Then $g$ is a new function from $\kappa\to\omega$. Moreover, for each limit ordinal $\alpha<\kappa$ we claim that the element $f_\alpha\in\omega^\omega$ defined by $f_\alpha(n)=g(\alpha+n)$ is new as well. Indeed, note that the restriction of $\PP$ to the interval $[\alpha,\alpha+\omega)$ is naturally isomorphic to $\Fn(\omega,\omega)$, and the restriction of $g$ to this interval is generic for it. Thus by the argument of Proposition~\ref{prop:generic-new}, $f_\alpha$ cannot be in $V$.
\end{proof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\section{More on the continuum}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% we can force CH
% cohen forcing over a model of CH leaves 2^omega exactly kappa (nice names)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\section{Random forcing}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% OLD STUFF ABOUT IDEALIZED COHEN FORCING
