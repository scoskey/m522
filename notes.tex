...
Note that forcing cannot do everything. For example, \ldots
...
When we add a new subset of $\mathbb P$ to the universe, we don't add an arbitrary set. We add a filter:
...
\begin{defn}
  A filter $G\subset\mathbb P$ is \emph{generic} if for every dense $D\subset\mathbb P$ we have $G\cap D\neq\emptyset$.
\end{defn}

Of course, generic filters don't exist---or do they???
