...
\begin{lem}
  A subset $A\subset\omega^\omega$ is compact if and only if it is closed and contained in some product of bounded intervals $\prod[0,K_n]$.
\end{lem}
...
\begin{proof}
  It is clear that $\aleph_1\leq\non(\mathcal K_\sigma)$ because every countable set is $\mathcal K_\sigma$, being the union of its points.

  \ldots

  Finally it is again clear that $\cov(\mathcal K_\sigma)\leq\mathfrak c$, since the whole space $\omega^\omega$ is the union of its points and $|\omega^\omega|=\mathfrak c$.
\end{proof}

In fact we'll see shortly that $\non(\mathcal K_\sigma)\leq\cov(\mathcal K_\sigma)$. Meavwhile the theorem implies the analog of the Baire category theorem for $\sigma$-compact sets: if $V_s$ is any basic open set then $V_s$ is not $\sigma$-compact. Indeed, observe that $V_s$ is homeomorphic to $\omega^\omega$ itself, and $\omega^\omega$ is not $\sigma$-compact because that would imply that $\cov(\mathcal K_\sigma)=1$.
...
The following result establishes the relationship between these two cardinal values.
