...
\begin{example}
  ...
  \]
  To check that it is a morphism, $\phi(x)\not\ni y$ implies $x-y\in A$, which in turn implies that $x\in y+A=\psi(y)$, as required. Recall that the function $\psi(x)=x+A$ featured prominently in our original proof: for each $X\notin\mathcal M$ we formed the family of translates $\{x+A:x\in X\}$.
  
  Once again, if we exchange the roles of $\phi$ and $\psi$ we obtain a morphism witnessing the inequality $\cov(\mathcal M)\leq\non(\mathcal N)$. Recall that this proof was the same, but using translates of the comeager null set $A^c$. The minus sign in $x-A^c$ has no effect on the argument.
\end{example}
...
\begin{rem}
  True inequalities between cardinal characteristics are not always witnessed by morphisms. For example, it is true that $\cov(\mathcal M)\leq\non(\mathcal{SN}$, but it is consistent that the dual inequality fails. [ref]
\end{rem}
...
\begin{exerc}
  Find a morphism behind the proof of the inequality $\mathfrak b\leq\non(\mathcal M)$ (Lemma~\ref{lem:b-nonm}). Check that it is dual to a morphism behind the inequality $\cov(\mathcal M)\leq\mathfrak d$.
\end{exerc}
...
If $x\in2^\omega$ is fixed and $P$ is an interval partition, we define the set $\Diff_P(x)$ to be the set of all $y\in2^\omega$ such that $y$ strongly differs from $x$ on $P$. Since an element $x\in2^\omega$ and an interval partition $P$ appear together very frequently, such a pair $(x,P)$ has been given the name \emph{chopped real}. The following result shows that the chopped reals give us a new, combinatorial basis for the meager ideal on Cantor space.
...
\begin{proof}
  We begin by writing $A=\bigcup A_n$ where each $A_n$ is nowhere dense and $A_n\subset A_{n+1}$ for all $n$. We will define the element $x\in2^\omega$ and the interval partition $P=(I_n)$ recursively. To begin, since $A_0$ isn't dense there exists $s\in2^{<\omega}$ such that $A_0\cap V_s=\emptyset$. We let $I_0=\dom(s)$ and $x\restriction I_0=s$.
...
The lemma implies that the collection of sets $\Diff_P(x)$ forms a basis for the meager ideal on Cantor space. This basis is particularly useful because it is easy to see when two sets of the form $\Diff_P(x)$ are related by the subset relation. 
...
The proof is elementary (assuming I have not made a typo) and we leave it for Exercise~\ref{exerc:engulf}. Proposition~\ref{prop:engulf} provides characterization of $\cof(\mathcal M)$ as the dominating number of a simple combinatorial relation on chopped reals. The cardinals $\mathfrak b$ and $\mathfrak d$ can be characterized by a similar combinatorial relation just on interval partitions. First, if $P=(I_n)$ and $Q=(J_m)$ are interval partitions, we write $P\prec Q$ if for all but finitely many $J_m$ there exists $I_m$ such that $I_n\subset J_m$. 

\begin{prop}
  We have $\mathfrak d$ is the dominating number of the $\prec$ relation and $\mathfrak b$ is the dominating number of the $\not\succ$ relation. In fact, there is a morphism from $\leq^*$ to $\prec$ and vice versa.
\end{prop}
...
\begin{lem}
  We have $\add(\mathcal M)\leq\mathfrak b$ and $\mathfrak d\leq\cof(\mathcal M)$.
\end{lem}

\begin{proof}
  We prove the second inequality by finding a morphism from the $\subset$ relation on $\mathcal M$ to the $\prec$ relation on interval partitions. We therefore obtain the first inequality as a consequence of duality. For this it is enough to consider the $\subset$ relation just on the basis of meager sets of the form $A=\Diff_Q(y)$. We let:
...
\begin{thm}
  We have $\add(\mathcal M)=\min\{\mathfrak b,\cov(\mathcal M)\}$.
\end{thm}

\begin{proof}
  We have already shown that $\add(\mathcal M)\leq\mathfrak b$ and $\add(\mathcal M)\leq\cov(\mathcal M)$. Hence it remains only to show that $\add(\mathcal M)\geq\min\{\mathfrak b,\cov(\mathcal M)\}$.
\end{proof}
